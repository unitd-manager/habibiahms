<?
class CPL_Admin_Widgets_Hms_InternalStockTransfer_Model extends CP_Common_Lib_WidgetModelAbstract
{
    /**
     *
     */
    ...
    function setSearchVar() {
        ...
        $searchVar->sqlSearchVar[] = "st.transfer_type = 'Internal'";
        $searchVar->sqlSearchVar[] = "st.status != 'Cancelled'";
        //$searchVar->sqlSearchVar[] = "(st.from_location = 1 OR st.to_location = 1)";
    ...
    function getDataArray() {
        ...
        $dataArray = $modelHelper->getWidgetDataArray($this->controller, 'hms_internalStockTransfer');
    ...
    function getExportToExcel(){
        ...
        $file_name = "InternalStockTransfer" . date("d-m-Y") . ".xls";
        ...
        LEFT JOIN (`po_product` pop) ON (pop.po_product_id = sth.po_product_id)
        LEFT JOIN (`product` p) ON (p.product_id = sth.product_id)
        WHERE st.transfer_type = 'Internal'
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $from['from_location']);
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $To['to_location']);
