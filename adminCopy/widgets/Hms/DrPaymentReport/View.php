...
class CPL_Admin_Widgets_Hms_DrPaymentReport_View extends CP_Common_Lib_WidgetViewAbstract
{
    ...
    function getRowsHTML() {
        ...
        if ($start_date != '' && $end_date == '') {
            $appendSql .= "AND pv.check_up_date BETWEEN '{$start_date}' AND '{$current_date}'";
        } else if ($start_date == '' && $end_date != ''){
            $appendSql .= "AND pv.check_up_date BETWEEN '{$start_date}' AND '{$end_date}'";
        } else if ($start_date != '' && $end_date != '') {
            $appendSql .= "AND pv.check_up_date BETWEEN '{$start_date}' AND '{$end_date}'";
        } else {
        ...
        foreach($this->model->dataArray as $row){
            ...
            GROUP BY DATE_FORMAT(pv.check_up_date, '%Y-%m'), ev.employee_id
            ";
        ...
        {$rows}
        <tr class=''>
            <td class='txtRight lastRowBgColor' colspan='1'>Total</td>
