...
class CPL_Admin_Widgets_Hms_VaccinationReport_Model extends CP_Common_Lib_WidgetModelAbstract
{
    ...
    function setSearchVar() {
        ...
        if ($due_date == 'Due Date') {
            ...
            if ($start_date != '' && $end_date == '') {
                $searchVar->sqlSearchVar[] = "vv.vaccination_date >= '{$start_date}' AND vv.vaccination_date <= '{$current_date}'";
            } else if ($start_date == '' && $end_date != ''){
                $start_date = $year . '-' . $month . '-' . '01';
                $searchVar->sqlSearchVar[] = "vv.vaccination_date >= '{$start_date}' AND vv.vaccination_date <= '{$end_date}'";
            } else if ($start_date != '' && $end_date != '') {
                $searchVar->sqlSearchVar[] = "vv.vaccination_date >= '{$start_date}' AND vv.vaccination_date <= '{$end_date}'";
            } else if ($monthVal == '' && $yearVal == ''){
                $start_date = $year . '-' . $month . '-' . '01';
            ...
            if ($monthVal != '') {
                $searchVar->sqlSearchVar[] = "DATE_FORMAT(vv.vaccination_date, '%m') = '{$monthVal}'" ;
            }
            if ($yearVal != '') {
                $searchVar->sqlSearchVar[] = "DATE_FORMAT(vv.vaccination_date, '%Y') = '{$yearVal}'" ;
            }
        }
