<?
class CPL_Admin_Modules_Hms_Order_Model extends CP_Admin_Modules_Hms_Order_Model
{
    function getSQL() {
        ...
        SELECT o.*
              ...
              ,ip.code
        FROM `order` o
         LEFT JOIN in_patient ip ON (o.in_patient_id = ip.in_patient_id)
        ";
        ...
        if ($order_id != "") {
            ...
            if ($order_type != '') {
               if ($order_type == 'POS_IP') {
                    $searchVar->sqlSearchVar[] = "o.in_patient_id != ''";
                } else  {
            ...
            if ($sales_return != "") {
                $searchVar->sqlSearchVar[] = "o.order_id IN (SELECT order_id from sales_return_history)";
            }
            ...
            if ($tv['keyword'] != "") {
                $searchVar->sqlSearchVar[] = "(
                    ...
                    OR o.cust_first_name  LIKE '%{$tv['keyword']}%'
                )";
            }
            ...
            if ($tv['special_search'] == "Today") {
                $searchVar->sqlSearchVar[] = "o.order_date ='{$today}'";
            }
    ...
    function getCancelInvoice() {
        ...
        //$resultIrh = $db->sql_query($SQLIrh); //To fetch row once again else no of rows return 0
        ...
        $modifiedBy = $fn->getSessionParam('userName');

        if ($numRowsIrh == 0) {
            /* Updating of invoice record */
            $sqlInv = "
            UPDATE invoice
            SET status = 'Cancelled', cancelled_date = '{$date}', cancelled_by = '{$modifiedBy}'
    ...
    function getCancelReceipt() {
        ...
        while ($rowRec = $db->sql_fetchrow($resultRec)) {
            $sqlUpdateInv = "
            UPDATE invoice
            SET status = 'Due'
            WHERE invoice_id = {$rowRec['invoice_id']}
            ";
            $resultUpdateInv = $db->sql_query($sqlUpdateInv);
        }
        ...
        WHERE receipt_id = '{$row['receipt_id']}'
        ";
        $resultRec1 = $db->sql_query($sqlRec1);
        ...
        if($orderRec['order_type'] != 'POS'){
            if($numRows > 0){
                if($orderRec['order_type'] == 'OP'){
                    ...
                    $SQLPVUpdate = "UPDATE in_patient SET status = 'Partial Receipt' WHERE in_patient_id = {$orderRec['in_patient_id']} {$appendSqlRec}";
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                } else{                
                    $SQLPVUpdate = "UPDATE lab_test SET status = 'Partial Receipt' WHERE lab_test_id = {$orderRec['lab_test_id']} {$appendSqlRec}";
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                }
            } else{
                if($orderRec['order_type'] == 'OP'){
                    $SQLPVUpdate = "UPDATE patient_visit SET status = 'Visited' WHERE patient_visit_id = {$orderRec['patient_visit_id']} {$appendSqlRec}";
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                } else if($orderRec['order_type'] == 'IP'){
                    $SQLPVUpdate = "UPDATE in_patient SET status = 'Admitted' WHERE in_patient_id = {$orderRec['in_patient_id']} {$appendSqlRec}";
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                } else{                
                    $SQLPVUpdate = "UPDATE lab_test SET status = 'Visited' WHERE lab_test_id = {$orderRec['lab_test_id']} {$appendSqlRec}";
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                }
            }
        }
        ...
        foreach ($invoiceItemIds as $key=>$value){
            ...
            if ($invoice_id > 0){
                ...
                if($rowII['record_type'] == 'POS'){
                    $faInv = array();
                    $faInv['status']   = 'Due';
                    $whereConditionInv = "WHERE invoice_id = {$rowII['invoice_id']}";
                    $sqlInvUpdate    = $dbUtil->getUpdateSQLStringFromArray($faInv, "invoice", $whereConditionInv);
                    $resultInvUpdate = $db->sql_query($sqlInvUpdate);

                    $fa1 = array();
                    $fa1['receipt_status'] = 'Paid';
                ...
                */
                ...
                AND product_id = '{$invoiceItemRec['record_id']}'
                {$sqlAppendSt}
                ";
                ...
                if($numRowsBatchWiseStock > 0) {
                    $faStock = array();
                    $faStock['sales_return_qty'] = $BatchWiseStock['sales_return_qty'] + $qty_return;
                    $faStock['current_stock']    = $BatchWiseStock['current_stock'] + $qty_return;
                    
                    $faStock        = $fn->addModificationDetailsToFieldsArray($faStock, 'inventory_batchwise_stock');
                    $whereCondition = "WHERE po_product_id = '{$invoiceItemRec['po_product_id']}'
                                       AND product_id = '{$invoiceItemRec['record_id']}'
                                       {$sqlAppendSt}";
                    ...
                    $SQLUpdateProduct = "
                    UPDATE product SET qty_in_stock{$siteIdForField} = IFNULL(qty_in_stock{$siteIdForField}, 0)+{$qty_return}
                    WHERE product_id = '{$invoiceItemRec['record_id']}'
                    ";
                    $resultUpdateProduct  = $db->sql_query($SQLUpdateProduct);
                    
                    $SQLUpdateInventory = "
                    UPDATE inventory SET actual_stock{$siteIdForField} = IFNULL(actual_stock{$siteIdForField}, 0)+{$qty_return}
                    WHERE product_id = '{$invoiceItemRec['record_id']}'
                    ";
     ...
     *
     */
      function getsearchinPatientCode() {
        $fn     = Zend_Registry::get('fn');
        $db     = Zend_Registry::get('db');
        $dbUtil = Zend_Registry::get('dbUtil');
        $cpCfg  = Zend_Registry::get('cpCfg');
        ...
        $inPatientCode = $extractor[0];
        
        
         $SQL = "
            SELECT ip.code AS value
            ,ip.code AS label
            ,ip.in_patient_id AS id
            FROM in_patient ip
            WHERE (ip.code LIKE '{$inPatientCode}%')
            ORDER BY ip.code
         ";
        $result = $db->sql_query($SQL);
     ...
     *
     */
    function getSearchPatientVisitCode() {
        $fn     = Zend_Registry::get('fn');
        $db     = Zend_Registry::get('db');
        $dbUtil = Zend_Registry::get('dbUtil');
        $cpCfg  = Zend_Registry::get('cpCfg');
        ...
        $SQL = "
        SELECT ip.visit_code AS value
              ,ip.visit_code AS label
              ,ip.patient_visit_id AS id
        FROM patient_visit ip
        WHERE (ip.visit_code LIKE '{$inPatientCode}%')
        ORDER BY ip.visit_code
        ";
        $result = $db->sql_query($SQL);
    ...
    function getUpdateInpatCodeOnOrder() {
       $validate = Zend_Registry::get('validate');
        ...
        $sqlUpdate = "
        UPDATE `order` SET in_patient_id = '{$ipRec['in_patient_id']}'
        WHERE order_id = '{$order_id}'
        ";
        $resultUpdate = $db->sql_query($sqlUpdate);
    ...
    function getUpdateVisitCodeOnOrder() {
       $validate = Zend_Registry::get('validate');
        ...
        $visit_code = $fn->getReqParam('visit_code');
        $order_id =  $fn->getReqParam('order_id');
        $patient_visit_id =  $fn->getReqParam('patient_visit_id');
        $ipRec = $fn->getRecordByCondition('patient_visit', "visit_code = '{$visit_code}'");
        $piRec = $fn->getRecordRowByID('patient_information', 'patient_information_id', $ipRec['patient_information_id']);

        if($visit_code == ''){
            $ipRec['patient_visit_id'] = '';
        }

        $sqlUpdate = "
        UPDATE `order` SET patient_visit_id = '{$ipRec['patient_visit_id']}', visit_code = '{$ipRec['visit_code']}', cust_first_name = '{$piRec['name']}'
        WHERE order_id = '{$order_id}'
        ";
        $resultUpdate = $db->sql_query($sqlUpdate);
        ...
        WHERE patient_visit_id = '{$ipRec['patient_visit_id']}'
        ";
    ...
    function getCancelBill(){
        ...
        while($rowII = $db->sql_fetchrow($resultInvoiceItem)) {
            ...
            SELECT po_product_id
                  ...
                  ,sales_qty
            FROM inventory_batchwise_stock
            WHERE po_product_id = '{$rowII['po_product_id']}'
            AND product_id = '{$rowII['record_id']}'
            {$sqlAppendSt}
            ";
            $resultBatchWiseStock  = $db->sql_query($SQLBatchWiseStock);
            $BatchWiseStock        = $db->sql_fetchrow($resultBatchWiseStock);
            ...
            if($numRowsBatchWiseStock > 0) {
                $SQLSalesReturn = "
                SELECT qty_return
                FROM sales_return_history
                WHERE invoice_id = {$rowII['invoice_id']}
                AND invoice_item_id = {$rowII['invoice_item_id']}
                AND order_id = {$order_id}
                ";
                $resultSalesReturn  = $db->sql_query($SQLSalesReturn);
                $rowSalesReturn     = $db->sql_fetchrow($resultSalesReturn);

                $qty = $rowII['qty'] - $rowSalesReturn['qty_return'];
                //$qty_return = $rowII['qty'];

                $faStock = array();
                $faStock['current_stock']    = $BatchWiseStock['current_stock'] + $qty;
                $faStock['sales_return_qty'] = $BatchWiseStock['sales_return_qty'] - $rowSalesReturn['qty_return'];
                $faStock['sales_qty']        = $BatchWiseStock['sales_qty'] - $rowII['qty'];
                $faStock        = $fn->addModificationDetailsToFieldsArray($faStock, 'inventory_batchwise_stock');
                $whereCondition = "WHERE po_product_id = '{$rowII['po_product_id']}'
                                   AND product_id = '{$rowII['record_id']}'
                                   {$sqlAppendSt}";
                $SQLStock       = $dbUtil->getUpdateSQLStringFromArray($faStock, "inventory_batchwise_stock", $whereCondition);
                $resultStock    = $db->sql_query($SQLStock);
                ...
                $SQLUpdateProduct = "
                UPDATE product SET qty_in_stock{$siteIdForField} = IFNULL(qty_in_stock{$siteIdForField}, 0)+{$qty}
                WHERE product_id = '{$rowII['record_id']}'
                ";
                $resultUpdateProduct  = $db->sql_query($SQLUpdateProduct);

                $SQLUpdateInventory = "
                UPDATE inventory SET actual_stock{$siteIdForField} = IFNULL(actual_stock{$siteIdForField}, 0)+{$qty}
                WHERE product_id = '{$rowII['record_id']}'
                ";
                $resultUpdateInventory  = $db->sql_query($SQLUpdateInventory);
            }
