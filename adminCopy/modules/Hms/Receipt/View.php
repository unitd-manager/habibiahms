<?
class CPL_Admin_Modules_Hms_Receipt_View extends CP_Admin_Modules_Hms_Receipt_View
{
    /**
     *
     */
    ...
    function getGenerateReceiptForm() {
            ...
            <div class='form-row-wrapper'>
                <div class='floatbox'>
                    <div class='float_left'>
                        <input type='checkbox' name='invoiceCode[]' invoice_id='{$row['invoice_id']}' value='{$row['invoice_id']}' class='invoiceCode'>
                    </div>
        ...
        $appensqlForDue = "";
        if($patient_visit_id != ""){
            $appensqlForDue .= " AND o.patient_visit_id < {$patient_visit_id}";
        }

        if($in_patient_id != ""){
            $appensqlForDue .= " AND o.in_patient_id < {$in_patient_id}";
        }

        if($lab_test_id != ""){
            $appensqlForDue .= " AND o.lab_test_id < {$lab_test_id}";
        }

        if($patient_information_id != ""){
            $appensqlForDue .= " AND o.patient_information_id < {$patient_information_id}";
        }
        ...
        if($appensqlForDue != "") {
            ...
            FROM invoice i
            LEFT JOIN `order` o ON (i.order_id = o.order_id)
            WHERE (i.status = 'Due' || i.status = 'Partial Payment') 
            {$appensqlForDue}
            ";
