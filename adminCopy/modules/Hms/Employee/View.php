...
class CPL_Admin_Modules_Hms_Employee_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            $timeoutdisplayHide = "";
            if($rowAttend['leave_time_day_shift'] != ""){
                $timeoutdisplayHide = "displayNone";
            }

            $nighttimeindisplayHide = "";
            if($rowAttend['time_in_night_shift'] != ""){
                $nighttimeindisplayHide = "displayNone";
            }

            $nighttimeoutdisplayHide = "";
            if($rowAttend['leave_time_night_shift'] != ""){
                $nighttimeoutdisplayHide = "displayNone";
            }

            $extratimeindisplayHide = "";
            if($rowAttend['time_in_extra_shift'] != ""){
                $extratimeindisplayHide = "displayNone";
            }

            $extratimeoutdisplayHide = "";
            if($rowAttend['leave_time_extra_shift'] != ""){
                $extratimeoutdisplayHide = "displayNone";
            }
            ...
            {$listObj->getListDataCell("<a class='btn btn-success {$timeindisplayHide} TimeinButton' href='#' employee_id={$row['employee_id']}>In</a> <a class='btn btn-success {$timeoutdisplayHide} TimeoutButton' href='#' employee_id={$row['employee_id']}>Out</a>")}
            {$listObj->getListDataCell("<a class='btn btn-success {$nighttimeindisplayHide} NightTimeinButton' href='#' employee_id={$row['employee_id']}>In</a> <a class='btn btn-success {$nighttimeoutdisplayHide} NightTimeoutButton' href='#' employee_id={$row['employee_id']}>Out</a>")}
            {$listObj->getListDataCell("<a class='btn btn-success {$extratimeindisplayHide} ExtraTimeinButton' href='#' employee_id={$row['employee_id']}>In</a> <a class='btn btn-success {$extratimeoutdisplayHide} ExtraTimeoutButton' href='#' employee_id={$row['employee_id']}>Out</a>")}
        ...
        {$listObj->getListHeaderCell('Time In / Time Out', '')}
        {$listObj->getListHeaderCell('Night Time IN / Out', '')}
        {$listObj->getListHeaderCell('Extra Time IN / Out', '')}
    ...
    function getEdit($row){
        ...
        $sqlEmployeeType     = $fn->getValueListSQL('employeeType');
        ...
        $normalTimings = "";
        $doubleShiftTimings = "displayNone";
        if($row['employee_type'] == 'Double Shift') {
            $normalTimings = "displayNone";
            $doubleShiftTimings = "";
        }

        $WorkingTimingDetails .= "
        <tr class='{$normalTimings} normalTimings'>
        ...
        //if($row['employee_type'] == 'Double Shift') {
        $WorkingTimingDetails .= "
        <tr class='{$doubleShiftTimings} doubleShiftTimings'>
            <th colspan='5'>Office Time Details</th>
        </tr>
        <tr class='{$doubleShiftTimings} doubleShiftTimings'>
            <th colspan='2'>DS Morning</th>
            <th colspan='3'>DS Evening</th>
        </tr>
        <tr class='{$doubleShiftTimings} doubleShiftTimings'>
            <td>{$formObj->getTimeRow('Time in (HH:MM)', 'time_in_morning', $row['time_in_morning'])}</td>
            <td>{$formObj->getTimeRow('Time out (HH:MM)', 'time_out_morning', $row['time_out_morning'])}</td>
            <td>{$formObj->getTimeRow('Time in (HH:MM)', 'time_in_evening', $row['time_in_evening'])}</td>
            <td>{$formObj->getTimeRow('Time out (HH:MM)', 'time_out_evening', $row['time_out_evening'])}</td>
            <td></td>
        </tr>
        <tr class='{$doubleShiftTimings} doubleShiftTimings'>
        ...
        <tr class='{$doubleShiftTimings} doubleShiftTimings'>
            <td>{$formObj->getTimeRow('Time in (HH:MM)', 'time_in_night1', $row['time_in_night'])}</td>
            <td>{$formObj->getTimeRow('Time out (HH:MM)', 'time_out_night1', $row['time_out_night'])}</td>
            <td colspan='3'></td>
        </tr>
        ";
        //}
        ...
        $feesFlds = '';
        if($row['position'] != 'Nurse' && $row['position'] != 'LAB TECHNICIAN' && $row['category'] != 'Staff' && $row['category'] != 'Student'){
            $feesFlds = "
            <td class='feesFlds'>{$formObj->getDDRowByArr('Fees Commission Type', 'fees_commission_type', $discountTypeArr, $row['fees_commission_type'])}</td>
            <td class='feesFlds'>{$formObj->getTBRow('Fees Commission', 'fees_commission', $row['fees_commission'])}</td>
            <td class='feesFlds'>{$formObj->getDDRowByArr('Lab Commission Type', 'lab_commission_type', $discountTypeArr, $row['lab_commission_type'])}</td>
            <td class='feesFlds'>{$formObj->getTBRow('Lab Commission', 'lab_commission', $row['lab_commission'])}</td>
            "; 
        }
        ...
        <div class='floatbox'>  
            ...
            <div class='mb10 float_right'><a href='#' class='btn btn-danger' id='deleteEmpRecord' employee_id={$row['employee_id']}>Delete</a></div>
        </div>
        {$formObj->getTBRow('', "error_box1", '', $expEdit)}
                    ...
                    <table class='thinlist'>
                        <tbody>
                            <tr>
                                ...
                                <td class=''>{$formObj->getYesNoRRow('Attendance Required', 'add_in_payroll', $row['add_in_payroll'])}</td>
                            ...
                            <tr>
                                {$feesFlds}
                                <td>{$formObj->getDDRowBySQL('Shift Type', 'employee_type', $sqlEmployeeType, $row['employee_type'], $expVl)}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>{$formObj->getDDRowBySQL('User Group', 'staff_user_group_id', $sqlUG, $row['staff_user_group_id'], $expUG)}</td>
                                <td>{$formObj->getYesNoRRow('Published', 'staff_published', $row['staff_published'])}</td>
                            </tr>
                            {$WorkingTimingDetails}
                            <tr>
    ...
    function getprintLeaveExtension() {
        ...
        <table border="0" width="100%" >
            <tr>
                <td align="center">LEAVE EXTENSION</td>
            </tr>
       ...
       <br/><br/><br/><br/>
        <table border="0" width="100%">
            <tr>
                <td width="30%" align="left">STAFF SIGN </td>
                <td width="50%" align="right">DR SIGN </td>
    ...
    function getprintLeaveForm() {
        ...
        <table border="0" width="100%" >
            <tr>
                <td align="center">LEAVE APPLICATION</td>
            </tr>
    ...
    function getQuickSearch() {
        ...
        $urlLeaveExtension = "index.php?module=hms_employee&_spAction=printLeaveExtension&employee_id={$employee_id}&showHTML=0";
            $printLeaveExtension = "<a href='{$urlLeaveExtension}' id='printPrescription' employee_id='{$employee_id}' class='btn btn-info' target='_blank'>Print Leave Extension</a>";

             $printLeaveForm = '';

        $urlLeaveForm = "index.php?module=hms_employee&_spAction=printLeaveForm&employee_id={employee_id}&showHTML=0";
            $printLeaveForm = "<a href='{$urlLeaveForm}' id='printPrescription' employee_id='{employee_id}' class='btn btn-info' target='_blank'>Print Leave Form</a>";
    ...
    function getEmployeeTimeInUpdate() {
        ...
        if ($numRowsStaffAtt == 0) {
            ...
            $fa['time_in_day_shift'] = date('H:i:s');
    ...
    function getEmployeeTimeOutUpdate() {
        ...
        if ($numRowsStaffAtt > 0) {
            ...
            $employeeRec = $fn->getRecordRowById('employee', 'employee_id', $employee_id);
            date_default_timezone_set("Asia/Calcutta");
            $fa['leave_time_day_shift'] = date('H:i:s');
    ...
    function getEmployeeNightTimeInUpdate() {
        ...
        if ($numRowsStaffAtt == 0) {
            ...
            $fa = array();
            date_default_timezone_set("Asia/Calcutta");
            $fa['time_in_night_shift'] = date('H:i:s');
            $fa['creation_date']     = date('Y-m-d H:i:s');
            $fa['employee_id']       = $employeeRec['employee_id'];
            $fa['record_date']       = $today;
            $fa['created_by']        = $employeeRec['first_name'] . ' ' . $employeeRec['last_name'];
            ...
            $whereCondition = "WHERE record_date = '{$today}' AND employee_id = '{$employeeRec['employee_id']}'";
            $SQL = $dbUtil->getUpdateSQLStringFromArray($fa, 'attendance', $whereCondition);
            $db->sql_query($SQL);            
        }
    ...
    function getEmployeeNightTimeOutUpdate() {
        ...
        if ($numRowsStaffAtt > 0) {
            ...
            $employeeRec = $fn->getRecordRowById('employee', 'employee_id', $employee_id);
            date_default_timezone_set("Asia/Calcutta");
            $fa['leave_time_night_shift'] = date('H:i:s');
            $fa['modification_date']    = date('Y-m-d H:i:s');
            $fa['modified_by']          = $employeeRec['first_name'] . ' ' . $employeeRec['last_name'];
    ...
    function getEmployeeExtraTimeInUpdate() {
        ...
        if ($numRowsStaffAtt == 0) {
            ...
            $fa = array();
            date_default_timezone_set("Asia/Calcutta");
            $fa['time_in_extra_shift'] = date('H:i:s');
            $fa['creation_date']     = date('Y-m-d H:i:s');
            $fa['employee_id']       = $employeeRec['employee_id'];
            $fa['record_date']       = $today;
            $fa['created_by']        = $employeeRec['first_name'] . ' ' . $employeeRec['last_name'];
    ...
    function getEmployeeExtraTimeOutUpdate() {
        ...
        if ($numRowsStaffAtt > 0) {
            ...
            $employeeRec = $fn->getRecordRowById('employee', 'employee_id', $employee_id);
            date_default_timezone_set("Asia/Calcutta");
            $fa['leave_time_extra_shift'] = date('H:i:s');
            $fa['modification_date']    = date('Y-m-d H:i:s');
            $fa['modified_by']          = $employeeRec['first_name'] . ' ' . $employeeRec['last_name'];
