...
cpm.hms.inPatient = {
    init: function(){
        ...
        $('.m-hms_inPatient input[name=continue_medicine]').livequery('click', function(){
            var parent = $(this).closest('tr');
            var rec_id = $(parent).attr('recid');
            var product_id = $(parent).attr('product_id');
            var continueMedicineObj = $(this).parents('tr').find('input[name=continue_medicine]').is(':checked');
            var continue_medicine = 0;
            if(continueMedicineObj == true){
                continue_medicine = 1;
            }
            var url = 'index.php?module=hms_inPatient&_spAction=updateProductLineItems&showHTML=0';
            $.get(url, {rec_id: rec_id, continue_medicine: continue_medicine, product_id: product_id}, function(json){

            });
        });

        $('.m-hms_inPatient input[name=treatment_given]').livequery('click', function(){
            var parent = $(this).closest('tr');
            var rec_id = $(parent).attr('recid');
            var product_id = $(parent).attr('product_id');
            var continueMedicineObj = $(this).parents('tr').find('input[name=treatment_given]').is(':checked');
            var treatment_given = 0;
            if(continueMedicineObj == true){
                treatment_given = 1;
            }
            var url = 'index.php?module=hms_inPatient&_spAction=updateProductLineItems&showHTML=0';
            $.get(url, {rec_id: rec_id, treatment_given: treatment_given, product_id: product_id}, function(json){
        ...
        $('.deleteMedicineRecord').livequery('click', function (){
            ...
            var medicines_in_patient_id = $(this).attr('medicines_in_patient_id');
            var in_patient_id   = $(this).attr('in_patient_id');
            $.get(url,  {medicines_in_patient_id:medicines_in_patient_id}, function(html){
                cpm.hms.inPatient.reloadMedicineTab(in_patient_id);
            });
        });
        ...
        $('.m-hms_inPatient .otherLabs').livequery('click', function (e){
            ...
            if(is_checked == true){
                var url = 'index.php?module=hms_inPatient&_spAction=medicalTestRecordUpdate&showHTML=0';
                Util.showProgressInd();
                $.get(url, {in_patient_id:in_patient_id, container:container,  lab_supplier_fees:lab_supplier_fees, supplier_id:supplier_id, medical_test_id:medical_test_id}, function(html){
                    Util.hideProgressInd();
       ...
       $('.viewSummaryForLabsRecord').live('click', function (e){
        ...
        $('input[name=dr_consultant_fees]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadTotalAmount();
        });
        ...
        $('input[name=advance_payment]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadTotalAmount();
        });

        $('input[name=anesthetic_fees]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=surgeon_fees]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=asst_surgeon_fees]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=theater_assistant_fees]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=theatre_charges]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=ot_charges]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });

        $('input[name=referral_charges]').livequery('keyup', function(){
            cpm.hms.inPatient.reloadSurgeryTotalAmount();
        });
    },
    ...
    reloadTotalAmount: function(){
        ...
        var dr_consultant_fees      = $('input[name=dr_consultant_fees]').val();
        ...
        total = parseFloat(consultation_fees) + parseFloat(amount) + parseFloat(nursing_fees)+ parseFloat(dr_consultant_fees) + parseFloat(other_fees) + parseFloat(advance_payment);

        $('.admissionTotalAmount').html(' - '+total+'RS');
    },

    reloadSurgeryTotalAmount: function(){
        var anesthetic_fees   = $('input[name=anesthetic_fees]').val();
        var surgeon_fees      = $('input[name=surgeon_fees]').val();
        var asst_surgeon_fees = $('input[name=asst_surgeon_fees]').val();
        var theatre_charges   = $('input[name=theatre_charges]').val();
        var ot_charges        = $('input[name=ot_charges]').val();
        var referral_charges  = $('input[name=referral_charges]').val();
        var theater_assistant_fees = $('input[name=theater_assistant_fees]').val();
        var total = parseFloat(0);
        ...
        if(theater_assistant_fees == "") {
            theater_assistant_fees = 0;
        }
        
        total = parseFloat(anesthetic_fees) + parseFloat(surgeon_fees) + parseFloat(asst_surgeon_fees) + parseFloat(theatre_charges) + parseFloat(ot_charges) + parseFloat(referral_charges) + parseFloat(theater_assistant_fees);
        $('.surgeryDetailsTotalAmount').html(' - '+total+'RS');
    },
