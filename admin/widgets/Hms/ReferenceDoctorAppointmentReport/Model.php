<?
class CPL_Admin_Widgets_Hms_ReferenceDoctorAppointmentReport_Model extends CP_Common_Lib_WidgetModelAbstract
{
    /**
     *
     */
...
function setSearchVar() {
    ...
    if ($start_date != '' && $end_date != '') {
        // If both start and end dates are provided, filter by the specified date range
        $searchVar->sqlSearchVar[] = "pv.appointment_date >= '{$start_date}' AND pv.appointment_date <= '{$end_date}'";
    } elseif ($start_date == '' && $end_date == '') {
        // If no date range is provided, filter for default current and future records
        $searchVar->sqlSearchVar[] = "(pv.appointment_date >= CURRENT_DATE)";
    }

    // Add other default conditions
    $searchVar->sqlSearchVar[] = "(pv.employee_id != '')";
    $searchVar->sqlSearchVar[] = "(pv.appointment_date != '')";
    ...
    function getDataArray() {
        ...
        $dataArray = $modelHelper->getWidgetDataArray($this->controller, 'hms_referenceDoctorAppointmentReport');
    ...
    function getExportToExcel(){
        ...
        $file_name = "ReferenceDoctorAppointmentReport_" . date("d-m-Y") . ".xls";
