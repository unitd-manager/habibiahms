...
cpm.hms.patientVisit = {
    init: function(){
        ...
        $('#Editequipmenthistory').live('click', function (e){
            var title = "Edit history";
            var patient_visit_id = $(this).attr('patient_visit_id');
            //var patient_visit_history_id = $(this).attr('patient_visit_history_id');
            e.preventDefault();
            var expObj = {
                validate: true
            ...
            Util.openFormInDialog.call(this, 'EditequipmenthistoryForm', title, 300, 282, expObj);
        });

        $('.deleteNoteshistory').livequery('click', function (e){
            var patient_visit_id = $(this).attr('patient_visit_id');
            var patient_information_id = $(this).attr('patient_information_id');

            msg = "If you want delete?";
            if (confirm (msg)){
                var url = 'index.php?module=hms_patientVisit&_spAction=deletehistory'
                    + '&showHTML=0';
                $.get(url, {patient_visit_id:patient_visit_id,patient_information_id: patient_information_id} ,function(html){
        ...
        $('.vaccinationFees').livequery('change', function(){
         ...
         $('.notReceivedLink').livequery('click', function(){
            
            var patient_visit_id = $(this).attr('patient_visit_id');

            var url = 'index.php?module=hms_patientVisit&_spAction=NotReceivedRecordUpdate&showHTML=0';
            $.get(url, {patient_visit_id:patient_visit_id}, function(html){
                    window.location.reload(true);
            });
        });

         $('.ReceivedLink').livequery('click', function(){
            
            var patient_visit_id = $(this).attr('patient_visit_id');

            var url = 'index.php?module=hms_patientVisit&_spAction=ReceivedRecordUpdate&showHTML=0';
            $.get(url, {patient_visit_id:patient_visit_id}, function(html){
                    window.location.reload(true);
            });
        });
        ...
        $('.vaccinationPortalDetail .dueDateVaccination .fld_date').livequery('change', function(){
            ...
            var conCatMedicalTestId = "";
            $('.m-hms_patientVisit .vaccinationPortalDetail input.vaccinationTitle:checked').each(function () {
                var checked_medical_test_id = $(this).attr('medical_test_id');
                var dueDateCheck = $('.dueDateVaccination input[name=due_date_'+checked_medical_test_id+']').val();
                if(dueDateCheck == "") {
                    if(due_date != "") {
                        var dateParent = $(".dueDateVaccination .row_due_date_"+checked_medical_test_id);
                        var res = due_date.split("-");
                        var dd = res[2];
                        var mm = res[1];
                        var y  = res[0];
             
                        var dueDateHidden = dd + '-'+ mm + '-' + y;
                        $('input[name=due_date_'+checked_medical_test_id+']', dateParent).val(due_date);
                        $('input[name=hidden_date_display]', dateParent).val(dueDateHidden);
                    }


                    if(conCatMedicalTestId != "") {
                        conCatMedicalTestId = conCatMedicalTestId + "_" + checked_medical_test_id;
                    } else {
                        conCatMedicalTestId = checked_medical_test_id;
                    }
                }
            });

            var url = 'index.php?module=hms_patientVisit&_spAction=updateVaccinationFeesAndDate&showHTML=0';
            $.get(url, {medical_test_id: medical_test_id, due_date: due_date, patient_visit_id:patient_visit_id, medical_test_id_due_date: conCatMedicalTestId}, function(json){
            });
        });
        ...
        $('.m-hms_patientVisit #addAppointmentRecord').livequery('click', function (e){
            ...
            var expObj = {
               ...
               ,callbackOnSuccess: function(){
                    ...
                    Util.alert(msg, function(){
                        Util.closeAllDialogs();
                        cpm.hms.patientVisit.reloadAppointmentTab(patient_visit_id);
                        //window.location.reload(true);
                    });
                }
            }
        ...
        $('.deleteAppointmentRecord').livequery('click', function (){
            Util.showProgressInd();
            var url = 'index.php?module=hms_patientVisit&_spAction=deleteAppointmentRecord&showHTML=0';
            var patient_appointment_id = $(this).attr('patient_appointment_id');
            var patient_visit_id   = $(this).attr('patient_visit_id');
            $.get(url,  {patient_appointment_id:patient_appointment_id}, function(html){
                cpm.hms.patientVisit.reloadAppointmentTab(patient_visit_id);
            });
        });

        $('.deleteAppointmentRecordList').livequery('click', function (){
            Util.showProgressInd();
            var url = 'index.php?module=hms_patientVisit&_spAction=deleteAppointmentRecord&showHTML=0';
            var patient_appointment_id = $(this).attr('patient_appointment_id');
            $.get(url,  {patient_appointment_id:patient_appointment_id}, function(html){
                cpm.hms.patientVisit.reloadAppointmentInList();
            });
        });
        ...
        $('.m-hms_patientVisit .viewAppointments').livequery('click', function (e){
            var title = "Appointments";
            e.preventDefault();

            var url = "index.php?module=hms_patientVisit&_spAction=appointmentsList&showHTML=0";
            var exp = {
                url: url
                ,afterOpen: function(){
            ...
            Util.openDialogForLink('Appointments', '700px', 'auto', 0, exp);
                ...
                var expObj = {
                   ...
                   ,callbackOnSuccess: function(){
                        ...
                        //Util.alert(msg, function(){
                            ...
                            Util.closeAllDialogs();
                            //var urlToList = 'index.php?_topRm=main&module=hms_patientVisit&_action=list';
                            //document.location = urlToList;

                            var PrintPrescription  = 'index.php?module=hms_patientVisit&_spAction=printPrescription&patient_visit_id='+patient_visit_id+'&showHTML=0';
                            window.open(PrintPrescription, '_blank');
                            window.location.reload(true);
                        //});
                    }
                }
        ...
        $('.m-hms_patientVisit .printLabel').livequery('click', function (e){
            var print_label = $(this).val();
            var is_checked  = $(this).is(':checked');
            var patient_visit_id = $(this).attr('patient_visit_id');
            var medical_test_id = $(this).attr('medical_test_id');
            ...
            if(is_checked == true){
                var url = 'index.php?module=hms_patientVisit&_spAction=medicalTestRecordPrintUpdate&showHTML=0';
                Util.showProgressInd();
                $.get(url, {print_label:print_label, patient_visit_id:patient_visit_id, medical_test_id:medical_test_id}, function(html){
                    Util.hideProgressInd();
                    $('.hideLabDetails', parent).show();
                    parent.addClass('bgColorHighlight');
                });
        ...
        $('.m-hms_patientVisit .otherLabs').livequery('click', function (e){
            var lab_supplier_fees = $(this).val();
            var is_checked  = $(this).is(':checked');
            var patient_visit_id = $(this).attr('patient_visit_id');
            var medical_test_id = $(this).attr('medical_test_id');
            ...
            if(is_checked == true){
                var update = 1;
                var url = 'index.php?module=hms_patientVisit&_spAction=medicalTestRecordUpdate&showHTML=0';
                Util.showProgressInd();
                $.get(url, {patient_visit_id:patient_visit_id, container:container,  lab_supplier_fees:lab_supplier_fees, supplier_id:supplier_id, medical_test_id:medical_test_id, update:update}, function(html){
                    Util.hideProgressInd();
                    $('.hideLabSupplierFees', parent).show();
                    cpm.hms.patientVisit.reloadReportReceived(0);
                });
            } else {
                var update = 0;
                var lab_supplier_fees = 0;
                var supplier_id = '';
                var container = '';
                var url = 'index.php?module=hms_patientVisit&_spAction=medicalTestRecordUpdate&showHTML=0';
                Util.showProgressInd();
                $.get(url, {patient_visit_id: patient_visit_id, container:container, lab_supplier_fees:lab_supplier_fees, supplier_id:supplier_id, medical_test_id:medical_test_id, update:update}, function(html){
                    Util.hideProgressInd();
                    $('.hideLabSupplierFees', parent).hide();
                    cpm.hms.patientVisit.reloadReportReceived(1);
                });
            }
        });
       ...
       $('.viewSummaryForLabsRecord').live('click', function (e){
        ...
        $('.viewOverallSummary').live('click', function (e){
            ...
            Util.openDialogForLink.call(this, 'Overall Summary', 850, 500, expObj);
        });

        $('.viewPatientSummary').live('click', function (e){
            e.preventDefault();
            var expObj = {
                beforeCloseFn: function(){
                    Util.closeAllDialogs();
            ...
            Util.openDialogForLink.call(this, 'In Patient Summary', 800, 400, expObj);
        });

        $('.viewNotesSummary').live('click', function (e){
            e.preventDefault();
            var expObj = {
                beforeCloseFn: function(){
                    Util.closeAllDialogs();
            ...
            Util.openDialogForLink.call(this, 'Notes Summary', 800, 400, expObj);
        });
        ...
        $("input[name='address_area_update']")
        .livequery(cpm.hms.patientVisit.patientAddress);

        $(".adminLoginAutoComplete input[name='address_area']")
        .livequery(cpm.hms.patientVisit.patientAddress);
        ...
        $(".createPatientVisitSearchButton").livequery("click", function (e){
            ...
            var blacklist              = $('input[name=blacklist]:checked').val();
            var diabetes               = $('input[name=diabetes]:checked').val();
            var partially_purchased    = $('input[name=partially_purchased]:checked').val();
            var not_purchased          = $('input[name=not_purchased]:checked').val();
            var not_paid_injection          = $('input[name=not_paid_injection]:checked').val();
            var not_purchased_medicine          = $('input[name=not_purchased_medicine]:checked').val();



            if(patient_name == ""){
                alert("Please Select/Enter Patient Details To Create Visit!");
                $('input[name=patient_name]').focus();
            } /*else if(address_area == "") {
                alert("Please Select Town/City!");
                $('input[name=address_area]').focus();
            }*/ else {         
                cpm.hms.patientVisit.createPatientVisitDetails(patient_information_id, '', patient_name, age_year, age_month, age_day, gender, father_name, husband_name, address_area, phone, weight, temperature, spo2, blacklist, diabetes, partially_purchased,not_paid_injection,not_purchased_medicine, not_purchased);
            }
        });

        $(".createPatientAppointmentSearchButton").livequery("click", function (e){
            var patient_information_id = $('input[name=patient_information_id]').val();
            var patient_visit_id       = $('#record_id').val();
            var patient_name           = $('input[name=patient_name]').val();
            var father_name            = $('input[name=father_name]').val();
            ...
            if(patient_name == ""){
                ...
                cpm.hms.patientVisit.createPatientAppointmentDetails(patient_information_id, '', patient_name, age_year, age_month, age_day, gender, father_name, husband_name, address_area, phone, weight, temperature, spo2, blacklist, diabetes, partially_purchased,not_paid_injection,not_purchased_medicine, not_purchased);
            }
        });
        ...
        /* Update town */
        $('#Updatetown').live('click', function (e){
                var patient_information_id = $(this).attr('patient_information_id');
                var title = "Update Town";
                e.preventDefault();

                var expObj = {
                    validate: true
                   ,callbackOnSuccess: function(){
                        var msg = 'Town Updated Successfully';
                        Util.alert(msg, function(){
                            Util.closeAllDialogs();
                            window.location.reload(true);
                        });
                ...
                Util.openFormInDialog.call(this, 'updateTownPortalForm', title, 600, 400, expObj);
        });
      
        $('#fld_referral_doctor_id').livequery('change', function(){
            var referralDoctorDropdown = document.getElementById('fld_referral_doctor_id');
            var reviewDateCell = document.getElementById('reviewDateCell');
    
            if (referralDoctorDropdown.value != '') {
                reviewDateCell.style.display = 'table-cell';
            } else {
                reviewDateCell.style.display = 'none';
            }
        });
        ...
        $('.cancelInvoiceReceipt').livequery('click', function(){
            var patient_visit_id = $('#record_id').val();
            var order_id         = $(this).attr('order_id');

            var msg = "Are you sure want to cancel all invoice and receipts ?";
            if (confirm(msg)){
                Util.showProgressInd();
                var url = 'index.php?module=hms_patientVisit&_spAction=cancelAllInvoicesAndReceipts&showHTML=0';
                $.get(url, {patient_visit_id: patient_visit_id, order_id: order_id}, function(){
                    Util.hideProgressInd();
                    
                    var mgsalert2 = 'All invoices and receipts are cancelled';
                    var n = noty({
                        text: mgsalert2,
                        type: 'confirm',
                        dismissQueue: true,
    ...
    reloadReportReceived: function(update){
        var url = 'index.php?module=hms_patientVisit&_spAction=reportReceivedFldFunction&showHTML=0';
        $.get(url, {update: update}, function(html){
            $('.reportReceivedFld').html(html);
            Util.hideProgressInd();
        });
    },
    ...
    reloadAppointmentTab: function(patient_visit_id){
        var url = 'index.php?module=hms_patientVisit&_spAction=appointmentPortalDisplay&showHTML=0';
        $.get(url, {patient_visit_id: patient_visit_id}, function(html){
            $('#appointmentDisplay').html(html);
            Util.hideProgressInd();
        });
    },

    reloadAppointmentInList: function(){
        var url = 'index.php?module=hms_patientVisit&_spAction=appointmentsList&showHTML=0';
        $.get(url, function(html){
            $('.appointmentListDisplay').html(html);
            Util.hideProgressInd();
        });
    },
    ...
    createPatientVisitDetails: function(patient_information_id, appointment_id, patient_name, age_year, age_month, age_day, gender, father_name, husband_name, address_area, phone, weight, temperature, spo2, blacklist, diabetes, partially_purchased,not_purchased_medicine,not_paid_injection, not_purchased){
        Util.showProgressInd();
        setTimeout(function (){
            Util.hideProgressInd();
            var address_area = $('input[name=address_area]').val();
            ...
            var url   = "index.php?module=hms_patientVisit&_spAction=selectDoctorDetails&patient_information_id="+patient_information_id
                        ...
                        +"&not_paid_injection="+not_paid_injection
                        +"&not_purchased_medicine="+not_purchased_medicine
    ...
    createPatientAppointmentDetails: function(patient_information_id, appointment_id, patient_name, age_year, age_month, age_day, gender, father_name, husband_name, address_area, phone, weight, temperature, spo2, blacklist, diabetes, partially_purchased,not_purchased_medicine,not_paid_injection, not_purchased){
        Util.showProgressInd();
        setTimeout(function (){
            Util.hideProgressInd();
            var address_area = $('input[name=address_area]').val();
            var title = "Choose Doctor";
            var url   = "index.php?module=hms_patientVisit&_spAction=selectDoctorForAppointment&patient_information_id="+patient_information_id
                        +"&appointment_id="+appointment_id
                        +"&patient_name="+patient_name
                        +"&father_name="+father_name
                        +"&husband_name="+husband_name
            ...
            var exp = {
               ...
               ,callbackOnSuccess: function(html){
                    ...
                    //cpm.hms.patientVisit.reloadQueueno();
                    var mgsalert2='Appointment Created Successfully';
                    var n = noty({
                        text: mgsalert2,
                        type: 'confirm',
                        dismissQueue: true,
    ...
    patientName: function() {
        var titleObj = this;
        var address_area    = $("input[name='address_area']").val();
        //var address_area    = $("select[name='address_area']").val();
        $(titleObj).autocomplete({
            source: function(request, response) {
                $.ajax({
                  url: 'index.php?module=hms_patientVisit&_spAction=searchPatientDetails&address_area='+address_area+'&showHTML=0',
                  dataType: "json",
                  data: request,                    
                  success: function (data) {
                    // No matching result
            ...
            select: function(event, ui) {
                ...
                var blacklist              = selectedObj.blacklist
                var diabetes               = selectedObj.diabetes
                var last_not_purchased               = selectedObj.last_not_purchased
                ...
                $('input[name=blacklist][value='+blacklist+']').prop('checked', true);
                $('input[name=diabetes][value='+diabetes+']').prop('checked', true);
                 $('input[name=not_purchased_medicine][value='+last_not_purchased+']').prop('checked', true);
            }
        });
    },
    ...
    patientAddress: function() {
        ...
        $(titleObj).autocomplete({
            ...
            select: function(event, ui) {
                ...
                var patient_information_id = selectedObj.id
                var city_town = selectedObj.city_town
                var url = 'index.php?module=hms_patientVisit&_spAction=updateTownInSession&showHTML=0';
                $.get(url, {city_town: city_town}, function(html){
                });
            }
        });
    },
    ...
    clearSearchValues: function() {
        ...
        $('select[name=gender]').val('Male');
        $('input[name=phone]').val('');
        $('input[name=blacklist][value=0]').prop('checked', true);
        $('input[name=diabetes][value=0]').prop('checked', true);

        var url = 'index.php?module=hms_patientVisit&_spAction=resetTownInSession&showHTML=0';
        Util.showProgressInd();
        $.get(url, function(html){
            Util.hideProgressInd();
        });
    ...
    //Sort the inverstigation with selected order
    sortInvestigationGroups: function() {
        var count = 0;
        $("#sortableInvesticationCategories").sortable({
            stop: function( event, ui ) {
                var valuelistId = [];
                $(".investigationCategoryLink").each(function() {
                    var href = $(this).prop('href');
                    var valuelist_id = $(this).attr('valuelist_id');
                    var name = $(this).text();
                    count++;
                    valuelistId.push(valuelist_id);
                });

                Util.showProgressInd();
                var url = 'index.php?module=hms_patientVisit&_spAction=investigationsCategorySortOrderUpdate&showHTML=0';
                $.get(url, {valuelistId: valuelistId}, function(html){
                    var patient_visit_id = $('#record_id').val();
                    cpm.hms.patientVisit.reloadInvestigationTabPortal(patient_visit_id);
                });
            }
