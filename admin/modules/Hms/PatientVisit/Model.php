...
class CPL_Admin_Modules_Hms_PatientVisit_Model extends CP_Common_Lib_ModuleModelAbstract
{
    ...
    function setSearchVar($linkRecType = '') {
        ...
        if ($_SESSION['userGroupName'] == 'LAB') {
            ...
            if ($check_up_date1 != "" && $check_up_date2 != "") {
                ...
                if ($yesterday == "Today") {
                    $searchVar->sqlSearchVar[] = "(pv.check_up_date BETWEEN '{$currentDate}' AND '{$currentDate}')";
                }
    ...
    function getSearchPatientDetails() {
        ...
        SELECT  p.name AS value
               ,CONCAT_WS(' :: ', p.name, p.father_name, p.spuse_name, p.address_area, p.phone) AS label
               ,p.patient_information_id AS id
               ,p.name AS Patient_Name
               ,p.father_name
               ,p.spuse_name AS husband_name
               ,p.address_area AS city_town
               ,p.age_year
               ,p.age_month
               ,p.age_day
               ,p.phone
               ...
               ,p.blacklist
               ,p.diabetes
               ,(SELECT MAX(not_purchased_medicine) 
                 FROM patient_visit pv 
                 WHERE pv.patient_information_id = p.patient_information_id) as last_not_purchased
        FROM patient_information p
        WHERE (p.patient_information_id LIKE '{$patientDetail}%'
        OR p.name LIKE '{$patientDetail}%')
        {$appendSql} 
