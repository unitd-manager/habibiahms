...
class CPL_Admin_Modules_Hms_PatientVisit_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getPatientVisitAppointmentSearchResult(){
        ...
        $empCondition = '';
        if($_SESSION['staff_id'] != '56' && $_SESSION['userGroupName'] != 'PHARMACY' && $_SESSION['userGroupName'] != 'Nurse' && $_SESSION['userGroupName'] != 'Super Administrator' && $_SESSION['userGroupName'] != 'LAB'){
            $recEmp       = $fn->getRecordByCondition("employee", "staff_id = {$_SESSION['staff_id']}
                AND status = 'Active'");
            $empCondition = "AND ev.employee_id = '{$recEmp['employee_id']}'";
        }
        ...
        if($numRows > 0){
            ...
            $resultRow = "
            ";
            if ($_SESSION['userGroupName'] == 'Administrator' || $_SESSION['userGroupName'] == 'Super Administrator' || $_SESSION['staff_id'] == '56'){
                $resultRow .= "
                <div class='searchResultLabel'>
...
if($row['status'] != 'Cancelled'){
    if ($_SESSION['userGroupName'] == 'Super Administrator' || $_SESSION['userGroupName'] == 'Administrator') {
        ...
        $weightText = '';
        if($row['age_year'] != ''){
            $age_year = intval($row['age_year']);
            $age_month = intval($row['age_month']);
            $age_actual = $age_year + ($age_month / 12);

            if($age_year >= 6 && $age_year <= 12){
                $actual_weight = round(($age_actual * 3) + 7, 1);
                $weightText = "<span class='blinking'>Weight Should Be : $actual_weight kg<b><span>";
            }
            else if($age_year >= 1 && $age_year <= 5){
                $actual_weight = round(($age_actual * 2) + 8, 1);
                $weightText = "<span class='blinking'>Weight Should Be : $actual_weight kg<b><span>";
            }
        }
        else if($row['age_month'] != '' && $row['age_year'] == ''){
            $actual_weight = round((intval($row['age_month']) + 9) / 2, 1);
            $weightText = "<span class='blinking'>Weight Should Be : $actual_weight kg<b><span>";
        }
        if($row['referral_doctor_id'] != '' ){
            $refData="
...
";
    ...
    ";
        ...
        if ($_SESSION['userGroupName'] == 'Administrator' || $_SESSION['userGroupName'] == 'Super Administrator'){
            ...
            ";
            $patInfoMainFld2 = "
            <td width='12%'>{$formObj->getTBRow('Husband Name', 'spuse_name', $row['spuse_name'])}</td>
            <td width='28%'>
            <div class='townBox'>{$formObj->getDDBSRowBySQL('Town/City', 'address_area', $sqlAddArea, $row['address_area'], $expGender)}</div>
        ...
        $FeesUpdate = '';
            if ($_SESSION['userGroupName'] == 'Administrator' || $_SESSION['userGroupName'] == 'Super Administrator' || $_SESSION['userGroupName'] == 'Nurse'){
                ...
                <div class=''>
                    <div class='type-check ym-fbox-check labTestBox {$bgColor}'>
                        ...
                        <div class='hideTreatmentDetails_{$row['title']}_{$count} hideLabDetails {$class} labVisitNotes'>
                            {$view}
                                <div>
                        <input type='checkbox' id='print_label_{$count}' value='1' name='print_label[]' class='printLabel' patient_visit_id='{$patient_visit_id}' medical_test_id='{$row['medical_test_id']}' ".($medTestVisitRec['print_label'] == 1 ? "checked='checked'" : "")." onclick=\"this.value = this.checked ? '1' : '0'\">
                        <label for='print_label_{$count}'>Print</label>
                    </div>
                            <div>
                                <label>Fees</label>
        ...
        <td>
           ...
           <td>
            <select name='staff_only'>
                ...
                {$cpUtil->getDropDown1($staffArray, $staff_only)}
           </select>
        </td>
        <td>
    ...
    function getPrintPrescription() {
       ...
       $tbl2 = '
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            else if($row['instruction'] == 'Weekly Once'){
