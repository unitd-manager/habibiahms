<?
class CPL_Admin_Modules_Hms_Receipt_Model extends CP_Admin_Modules_Hms_Receipt_Model
{
    /**
     *
     */
    ...
    function getGenerateReceiptFormSubmit() {
        ...
        foreach($invoiceCodes AS $invoice_id){
            $appendSqlInvoice = '';
            if ($cpCfg['cp.hasMultiUniqueSites']) {
            ...
            if($count == 0){
                $invoice_id_main = $invoiceRec['invoice_id'];
            }
            ...
            $appendSqlPaid2 = '';
            if ($cpCfg['cp.hasMultiUniqueSites']) {
                $appendSqlPaid1 = " AND r.site_id = {$cpSiteIdSession}";
            ...
            $faHist = array();
            $faHist['receipt_id']         = $receipt_id;
            $faHist['invoice_id']         = $invoice_id;
            $faHist['amount']             = $recpInvAmount;
            $faHist['related_invoice_id'] = $invoice_id;
            $faHist['creation_date']      = date("Y-m-d H:i:s");
            $fn->addRecord($faHist, 'invoice_receipt_history');
            $count++;
        }
    ...
    function getGenerateReceiptForMedia($receipt_id, $invoiceCodes) {
        ...
        $fa['room_name']        = 'tradingin_receipt';
        $fa['lang']             = 'eng';
