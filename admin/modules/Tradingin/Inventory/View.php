    ...
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            */
            ...
            $stockColumn = "";
            $avgColumn = "";
            if ($_SESSION['userGroupName'] == "Super Administrator" || $_SESSION['userGroupName'] == "Administrator" || $_SESSION['userGroupName'] == "Duty Doctor Admin") {
                $adjust_stock = "
            ...
            $manual_stock_date = $fn->getCPDate($rowMS['date'], 'd-m');
            $manual_stock_time = $fn->getCPDate($rowMS['time'], 'H:i');

            $msdt = '';
            if ($manual_stock_date != ''){
                $msdt = "({$manual_stock_date} / {$manual_stock_time})";
            }
            $msClass = '';
            if ($rowMS['date'] != $currentDate){
                $msClass = "redColor";
            }
            $manual_stock = "
            <a class='manualStock {$msClass} ml10' product_id='{$row['product_id']}' actual_stock='{$actual_stock}' site_id='{$site_id}' >
                Manual {$msdt}
            </a>
            ";
            ...
            if ($tv['special_search'] == "Rack Wise SD") {
                $rackDisplay = $listObj->getListDataCell($row['rake']);
            }
            ...
            if ($expiry_date != ''){
                ...
                $SQLPOP = "
                SELECT po.expiry_date
                      ,po.batch_no
                FROM inventory_batchwise_stock ibs
                LEFT JOIN po_product po ON (po.po_product_id = ibs.po_product_id)
                WHERE ibs.current_stock  - IFNULL(po.return_qty_ns, 0)> 0
                AND ibs.site_id = {$cpSiteIdSession}
                AND ibs.product_id = {$row['product_id']}
                AND DATEDIFF(po.expiry_date, Now()) < '{$expiryDay}'
                ";
                $resultPOP = $db->sql_query($SQLPOP);
                $batchDate = '';
                while($rowPOP = $db->sql_fetchrow($resultPOP)){
                    $expiryDate = $fn->getCPDate($rowPOP['expiry_date'], 'd-m-Y');
                    $batchDate .= $expiryDate.' / '.$rowPOP['batch_no'].'<br/>';
                }
                
                $batchDateDisplay = $listObj->getListDataCell($batchDate);
            }
           
            $medPurchQty = "
            {$formObj->getTBRow('', 'med_purch_qty', number_format($row['med_purch_qty'.$site_id]))}
            <a class='btn btn-success saveMedPurchQty' site_id='{$site_id}' inventory_id='{$row['inventory_id']}'>
            ...
            if ($tv['special_search'] == "Flagged") {
                $medPurchQtyDisplay = $listObj->getListDataCell($medPurchQty);
            }

            $expQty = "";
            if ($_SESSION['userGroupName'] == "Super Administrator") {
                $expQty = "
                {$formObj->getTBRow('', 'exp_qty', $row['exp_qty'.$site_id])}
                <a class='btn btn-success saveExpQty' site_id='{$site_id}' inventory_id='{$row['inventory_id']}'>
                    <span class='glyphicon glyphicon-floppy-disk'></span>
                </a>
                ";
            } else {
                $expQty = $row['exp_qty'.$site_id];
            }

            if($row['flag'.$site_id] == 1){
                $flgImg = "
                <a href='#' class='inv-list-flag' module='tradingin_inventory' record_id='{$row['inventory_id']}' currentvalue='1' color='red'>
                    <img src='/cmspilotv30/CP/common/images/icons/flag_on_red.png'>
                </a>
                ";
            } else {
                $flgImg = "
                <a href='#' class='inv-list-flag' module='tradingin_inventory' record_id='{$row['inventory_id']}' currentvalue='0' color='red'>
                    <img src='/cmspilotv30/CP/common/images/icons/flag_off.png'>
                </a>
                ";                
            }

            $companyName = substr($rowPO['company_name'],0,3);
            $companyNameArr = explode(' ', $rowPO['company_name']);
            $companyName1 = substr($companyNameArr[1],0,3);
            ...
            {$productCodeTd}
            {$listObj->getGoToDetailText($count, $row['product_name'] . " ({$percentageCal}%) ({$companyName} {$companyName1})")}
            {$stockColumn}
            {$medPurchQtyDisplay}
            {$manualStockTodayDisplay}
            {$manualStockDateDisplay}
            ...
            {$adjustStockColumn}
            {$listObj->getListDataCell($row['minimum_order_level'.$site_id])}
            {$listObj->getListDataCell($expQty)}
     ...
     if ($_SESSION['userGroupName'] == "PHARMACY" ) {
        ...
        $rackLblDisplay = '';
        if ($tv['special_search'] == "Rack Wise SD") {
            $rackLblDisplay = $listObj->getListHeaderCell('Rack', '');
        }

        $medPurchQtyLblDisplay = '';
        if ($tv['special_search'] == "Flagged") {
            $medPurchQtyLblDisplay = $listObj->getListHeaderCell('Med Purch Qty', '');
        }

        $batchDateLblDisplay = '';
        if ($expiry_date != '') {
            $batchDateLblDisplay = $listObj->getListHeaderCell('Expiry / Batch', '');
        }

        $text = "
        {$listObj->getListHeader()}
        <th width='25' align='center'>
            <a href='#' class='inv-list-flag-all' color='red'>
                <img src='/cmspilotv30/CP/common/images/icons/flag_on_red.png' border='0'>
            </a>
            <a href='#' class='inv-list-unflag-all' color='red'>
                <img src='/cmspilotv30/CP/common/images/icons/flag_off.png' border='0'>
            </a>
        </th>
        {$productCodeTh}
        ...
        {$adjustStockColumnHeading}
        {$listObj->getListHeaderCell('MOL', 'i.minimum_order_level'.$site_id )}
        {$listObj->getListHeaderCell('EXP Qty', 'i.exp_qty'.$site_id)}
                    ...
                    <table class='thinlist'>
                        <tbody>
                            <tr>
                                ...
                                <td width='25%'>{$formObj->getTBRow('MOL', 'minimum_order_level'.$site_id, $row['minimum_order_level'.$site_id])}</td>
...
//           ,"Past Stock Difference"
       ...
       if ($tv['category_id'] != "") {
        ...
        $sqlValuelist = "
        SELECT c.value,c.valuelist_id
              
        FROM valuelist c
        WHERE key_text = 'productType' 
        ";
        ...
        $printFlaggedMedicine = "<a href='{$urlFlaggedMedicine}' id='printFlaggedMedicine' class='btn btn-info' target='_blank'>Print PO Medicine</a>";
     	...
     	$urlConsultantMedicine = "index.php?module=tradingin_inventory&_spAction=printConsultantMedicine&showHTML=0&employee_id={$employee_id}";

        $printConsultantMedicine = "<a href='{$urlConsultantMedicine}' id='printConsultantMedicine' class='btn btn-info' target='_blank'>Print Cons Medicine</a>";

        $text = "
           <td class='fieldValue'>
            <select name='product_type'>
                <option value=''>Product Type</option>
                {$dbUtil->getDropDownFromSQLCols1($db, $sqlValuelist, $product_type)}
            </select>
        </td>
           <td class='fieldValue'>
    ...
    function getBatchWiseStockDisplayDetail($inventory_id, $product_id, $site_id){
        ...
        $SQLUpdate = "UPDATE inventory_batchwise_stock SET current_stock = 0 WHERE current_stock < 0";
        $result1 = $db->sql_query($SQLUpdate);
  ...
  $amountlg = '';
               ...
               $amountlg= "<td class='txtRight'>{$rowOI['unit_price']}</td>";
        }

           $amountlg1 = '';

                        if ($_SESSION['userGroupName'] != "Nurse") {
               $amountlg1= "<th class='txtRight'>Amount</th>";
        }
                $rows .= "
                <tr class='orderRightPanelTr'>
                    <td>
   ...
   function getPrintConsultantMedicine($employee_id) {
        $db = Zend_Registry::get('db');
        $fn = Zend_Registry::get('fn');
        $cpCfg = Zend_Registry::get('cpCfg');
        ...
        FROM inventory i
        LEFT JOIN (po_product pop) ON (pop.product_id = i.product_id)
        LEFT JOIN (purchase_order po) ON (po.purchase_order_id = pop.purchase_order_id)
        ...
        WHERE i.product_id IN (
         ...
         AND po.status != 'Cancelled'
         AND po.purchase_order_date >= CURDATE() - INTERVAL 2 YEAR
        ...
        $today = date("d-m-Y");
        $proRec = $fn->getRecordRowByID('employee', 'employee_id', $employee_id);
        ...
        $tbl3 = '<table border="0" width="100%" cellpadding="4" style="border-top:1px solid #000000;">';
   			   ...
   			   $tbl3 .=
                    '<tr >
                        <td align="left" width="100%"><br/><br/>Consultant Name : '.$proRec['first_name'].'</td>
                    </tr>';

        $tbl3 .= '<tr>
                    <td border="1" align="left" width="65%">Name</td>
                    <td border="1" align="left" width="35%">Qty</td>
                  </tr>';
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            SELECT pop.po_product_id
                  ...
                  ,pop.qty
                  ,p.title
                  ,i.med_purch_qty{$cpSiteIdSession}
            FROM po_product pop
            LEFT JOIN (purchase_order po) ON (po.purchase_order_id = pop.purchase_order_id)
            LEFT JOIN (inventory i) ON (i.product_id = pop.product_id)
            LEFT JOIN (product p) ON (p.product_id = pop.product_id)
            WHERE po.company_id_supplier = '{$row['company_id_supplier']}'
              AND i.product_id IN (
                    SELECT mv.product_id
                    FROM consultant_doctor mv
                    WHERE mv.employee_id = '{$employee_id}'
            ...
            while ($rowPo = $db->sql_fetchrow($resultPo)) {    
                if (!in_array($rowPo['title'], $productArray)) {
                    $tbl3 .= 
                    '<tr>
                        <td border="1" align="left" width="65%">' . $rowPo['title'] . '</td>
                        <td border="1" align="left" width="35%">' . number_format($rowPo['med_purch_qty' . $cpSiteIdSession]) . '</td>
                    </tr>';
                    array_push($productArray, $rowPo['title']);
                }
            }
    ...
    function getPrintFlaggedMedicine() {
        ...
        LEFT JOIN (purchase_order po) ON (po.purchase_order_id = pop.purchase_order_id)
        LEFT JOIN (supplier s) ON (s.supplier_id = po.company_id_supplier)
        WHERE po.company_id_supplier IN (3, 4, 6, 12, 14, 5, 17, 13)
         AND i.flag{$cpSiteIdSession} = 1
         AND po.status != 'Cancelled'
         AND po.purchase_order_date >= CURDATE() - INTERVAL 2 YEAR
        GROUP BY po.company_id_supplier)
        UNION
        ...
        LEFT JOIN (purchase_order po) ON (po.purchase_order_id = pop.purchase_order_id)
        LEFT JOIN (supplier s) ON (s.supplier_id = po.company_id_supplier)
        WHERE po.company_id_supplier NOT IN (3, 4, 6, 12, 14, 5, 17, 13)
         AND i.flag{$cpSiteIdSession} = 1
         AND po.status != 'Cancelled'
         AND po.purchase_order_date >= CURDATE() - INTERVAL 2 YEAR
        GROUP BY po.company_id_supplier)
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if($cpSiteIdSession == 1)
            {
                if(in_array($row['company_id_supplier'], array(3, 4, 6, 12, 14, 5 , 17, 13)))
                {
                    $tbl3 = $tbl3.
                    '<tr >
                        <td align="left" width="100%"><br/><br/>Supplier : '.$row['company_name'].'</td>
                    </tr>';
            ...
            while ($rowPo = $db->sql_fetchrow($resultPo)) {    
            //3- siva pharma, 4 - siva agen, 6 - siva enter , 12 - sankar, 14- var, 17-bio, 5 -nulife , 13 - fours           
                if($cpSiteIdSession == 1 )
                {
                    if (!in_array($rowPo['title'], $productArray)) {
                        $tbl3 = $tbl3.
