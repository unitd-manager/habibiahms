...
class CPL_Admin_Modules_Tradingsg_Supplier_Model extends CP_Common_Lib_ModuleModelAbstract
{
        ...
        if ($supplier_id != "") {
            ...
            if ($tv['keyword'] != "") {
                $searchVar->sqlSearchVar[] = "(
                    ...
                    OR s.phone       LIKE '%{$tv['keyword']}%'
                    OR s.contact_phone LIKE '%{$tv['keyword']}%'
                )";
            }
    ...
    function getGeneratePurchaseOrderFormSubmit() {
        ...
        $siteId          = $fn->getPostParam('siteId', array());
        $amount          = $fn->getPostParam('totalAmountPo');
        ...
        foreach($poCodes AS $po_code){
            $site_id = $siteId[$count];

            $appendSqlPo = '';
            if ($cpCfg['cp.hasMultiUniqueSites']) {
                $appendSqlPo = "AND site_id = {$site_id}";
            }
            ...
            if($PORec['overall_discount'] == '') {
                $PORec['overall_discount'] = 0;
            }

            if($PORec['purchase_return'] == '') {
                $PORec['purchase_return'] = 0;
            }
            ...
            SELECT SUM(pop.qty * pop.cost_price) AS total_cost
                  ,SUM((((pop.qty * pop.cost_price) * pop.discount_percentage) / 100) + (((pop.qty * pop.cost_price) * {$PORec['overall_discount']}) / 100)) AS Discount_Total
                  ,SUM((((pop.qty * pop.cost_price) - (((pop.qty * pop.cost_price) * pop.discount_percentage) / 100) - (((pop.qty * pop.cost_price) * {$PORec['overall_discount']}) / 100)) * pop.gst) / 100) AS GST_Total
            ...
            $totalCost   = $rowTotal['total_cost'] - $rowTotal['Discount_Total'] + $rowTotal['GST_Total'] - $PORec['purchase_return'];

            $po_amount = round($totalCost);
    ...
    function getGeneratePurchaseOrderFormValidate() {
        ...
        if ($poCodes != ''){
            $SQL = "
                SELECT SUM(pop.qty*pop.selling_price) AS po_amount
                FROM purchase_order p
                LEFT JOIN po_product pop ON (pop.purchase_order_id = p.purchase_order_id)
                WHERE p.po_code IN ({$po_code})
            ";
        ...
        if($amount == ""){
            $validate->validateData('amount' , 'Please check the amount');
        }
    ...
    function getPopulatePOAmount() {
        ...
        $rowDisc = $fn->getRecordRowByID('purchase_order', 'purchase_order_id', $purchaseOrderId);
        $overall_discount = $rowDisc['overall_discount'];

        if($overall_discount == ''){
            $overall_discount = 0;
        }

        $purchase_return = $rowDisc['purchase_return'];
        ...
        FROM po_product pop WHERE pop.purchase_order_id IN ({$purchase_order_id})
        ";
        $resultPaid = $db->sql_query($SQLPaid);
        $rowPaid    = $db->sql_fetchrow($resultPaid);
        $totalCost   = $rowPaid['po_amount'] - $rowPaid['Discount_Total'] + $rowPaid['GST_Total'] - $overall_discount - $purchase_return;
        ...
        if ($rowPartialPayment['Po_partial_payment'] == 0){
            ...
            return $totalCost - round($rowPartialPayment['Po_partial_payment']);
        }
    ...
    function getPopulatePOTotalAmount() {
        ...
        SELECT SUM(pop.qty * pop.cost_price) AS po_amount
              ,SUM((((pop.qty * pop.cost_price) * pop.discount_percentage) / 100) + (((pop.qty * pop.cost_price) * {$overall_discount}) / 100)) AS Discount_Total
              ,SUM((((pop.qty * pop.cost_price) - (((pop.qty * pop.cost_price) * pop.discount_percentage) / 100) - (((pop.qty * pop.cost_price) * {$overall_discount}) / 100)) * pop.gst) / 100) AS GST_Total
        FROM po_product pop WHERE pop.supplier_id IN ({$supplier_id})
        ";
        $resultPaid = $db->sql_query($SQLPaid);
        $rowPaid    = $db->sql_fetchrow($resultPaid);
        $totalCost   += $rowPaid['po_amount'] - $rowPaid['Discount_Total'] + $rowPaid['GST_Total'] - $overall_discount - $purchase_return;
