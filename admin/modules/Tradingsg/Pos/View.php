...
class CPL_Admin_Modules_Tradingsg_Pos_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getList($dataArray){
        ...
        if($session_order_id != ''){
            ...
            <div class='float_left'>
                <a class='btn btn-danger deleteAllOrderItemButton {$disableBtn}'>Delete All</a>
            </div>
            ";

            $sqlCode = "
            SELECT CONCAT_WS(' - ', ip.code, p.name) AS name
            FROM in_patient ip
            ...
            $ipRec = $fn->getRecordRowByID('in_patient', 'in_patient_id', $rowOrder['in_patient_id']);
            $piRec = $fn->getRecordRowByID('patient_information', 'patient_information_id', $ipRec['patient_information_id']);

            $sqlVisitCode = "
            SELECT CONCAT_WS(' - ', ip.visit_code, p.name) AS name
            FROM patient_visit ip
            LEFT JOIN (patient_information p) ON (p.patient_information_id = ip.patient_information_id)
            WHERE ip.visit_code != ''
            ORDER BY ip.visit_code DESC
            ";
            $pvRec = $fn->getRecordRowByID('patient_visit', 'patient_visit_id', $rowOrder['patient_visit_id']);
            $pvpiRec = $fn->getRecordRowByID('patient_information', 'patient_information_id', $pvRec['patient_information_id']);

            $patientNameSave = "
            <table width='100%' cellpadding='4'>
                <tr>
                    <td>
                        <div class='float_left nameSaveInPOS'>
                            <input type='text' name='patient_name' placeholder='Name' class='patientNameSave' value='{$rowOrder['cust_first_name']}'>
                        </div>
                        <div class='float_left nameSaveInPOS visitCodeSearch'>
                            <input type='text' value='{$rowOrder['visit_code']}' id='fld_visit_code' class='text' name='visit_code' placeholder='Visit Code'>
                        </div>

                        <div class='float_left codeSaveInPOS'>
                            {$formObj->getDDBSRowBySQL('IP Code',  'code', $sqlCode, $ipRec['code'].' - '.$piRec['name'], $exp)}

                        </div>
                        <div  class='float_left counterSaveInPOS'>
                            {$formObj->getYesNoRRow('Counter Sales', 'counter_sale', $rowOrder['counter'])}
                        </div>
                        <div>
                        </br>
                        </div>
                        <div class='float_left nameSaveButtonInPOS'>
        ...
        //if(strpos($_SERVER['HTTP_HOST'], 'localhost') || strpos($_SERVER['HTTP_HOST'], 'testpilotweb')){
            ...
            $thermalPrinter ="
            <div class='btn btn-info float_right printPosBtn displayNone'>
        ...
        $viewAllOrderLink = "index.php?module=hms_order&searchDone=1&order_type=POS";
                ...
                <table class='list thinlist'>
                    <thead>
                        <tr class='txt_16px'>
                            ...
                            <th width='9%' class='txtCenter'>Exlude-Stock</th>
                            <th width='2%' class='txtCenter'>Rack</th>
                            <th width='25%'>{$ln->gd('cp.label.itemName', 'Medicine Name')}</th>
                            <th width='5%'>{$ln->gd('cp.label.batch_no','Batch No')}</th>
                            {$product_code}
                            ...
                            <th width='8%' class='txtRight'>{$ln->gd('cp.label.unitPrice','Unit Price')}</th>
                            <!--<th width='8%'> 
    ...
    function getOrderItems(){
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $appendSqlSiteStockIbs = '';
            if($cpCfg['cp.hasMultiUniqueSites']) {
                $appendSqlSiteStockIbs = " AND ibs.site_id = {$cpSiteIdSession}";
            }
            ...
            AND ibs.current_stock > 0
              {$appendSqlSiteStockIbs}
            ";
            $resultCheckBatchCount  = $db->sql_query($SQLCheckBatchCount);
            $numRowsCheckBatchCount = $db->sql_numrows($resultCheckBatchCount);
            $rowCheckBatchCount     = $db->sql_fetchrow($resultCheckBatchCount);
            
            if($row['po_product_id'] != "") {
                if($numRowsCheckBatchCount <= 1) {
                    $SQLCheckStock = "
            ...
            if($numRowsCheckBatchCount > 1) {
                $appendSqlSiteStockIbs2 = '';
                if($cpCfg['cp.hasMultiUniqueSites']) {
                    $appendSqlSiteStockIbs2 = " AND ibs2.site_id = {$cpSiteIdSession}";
                }

                $SQLInventoryBatchStock = "
                SELECT ibs.inventory_batchwise_stock_id
                FROM inventory_batchwise_stock ibs
                WHERE ibs.product_id    = '{$row['record_id']}'
                  AND ibs.po_product_id = '{$row['po_product_id']}'
                  {$appendSqlSiteStockIbs}
                ";
                $resultInventoryBatchStock = $db->sql_query($SQLInventoryBatchStock);
                $rowInventoryBatchStock    = $db->sql_fetchrow($resultInventoryBatchStock);

                $SQLStockPrev ="
                SELECT SUM(ibs2.current_stock) AS stockCheck
                FROM inventory_batchwise_stock ibs2 
                LEFT JOIN (po_product pop) ON (pop.po_product_id = ibs2.po_product_id)
                LEFT JOIN (purchase_order pur) ON (pur.purchase_order_id = ibs2.purchase_order_id)
                WHERE ibs2.product_id = '{$row['record_id']}'
                  AND ibs2.inventory_batchwise_stock_id < '{$rowInventoryBatchStock['inventory_batchwise_stock_id']}'              AND pur.status != 'Cancelled'
                  AND ibs2.current_stock != 0
                  AND pur.status != 'Cancelled'
                  {$appendSqlSiteStockIbs2}
                ORDER BY ibs2.inventory_batchwise_stock_id ASC
                ";
                $resultStockPrev = $db->sql_query($SQLStockPrev);
                $rowStockPrev    = $db->sql_fetchrow($resultStockPrev);

                if($rowStockPrev['stockCheck'] < 0) {
                    $stock = $stock + $rowStockPrev['stockCheck'];
                } else {
                    $SQLPO ="
                    SELECT ibs.inventory_batchwise_stock_id
                    FROM inventory_batchwise_stock ibs
                    ...
                    WHERE ibs.product_id = '{$row['record_id']}'
                      ...
                      {$appendSqlSiteStockIbs}
                    ORDER BY ibs.inventory_batchwise_stock_id ASC
                    ";
                    ...
                    if($numRowsPo == 1) {
                        ...
                        LEFT JOIN (purchase_order pur) ON (pur.purchase_order_id = ibs2.purchase_order_id)
                        WHERE ibs2.product_id = '{$row['record_id']}'
                          AND ibs2.inventory_batchwise_stock_id < '{$rowInventoryBatchStock['inventory_batchwise_stock_id']}'
                          {$appendSqlSiteStockIbs2}
                          AND pur.status != 'Cancelled'
                          AND ibs2.current_stock != 0
                        ...
                        ORDER BY ibs2.inventory_batchwise_stock_id ASC
                        ";
                        $resultStockPrev2 = $db->sql_query($SQLStockPrev2);
                        $rowStockPrev2    = $db->sql_fetchrow($resultStockPrev2);
                        
                        $stock = $stock + $rowStockPrev2['stockCheck'];
                    }
                }
            }
            ...
            if($row['expiry_date'] != "" && $row['expiry_date'] != "0000-00-00") {
                if($row['expiry_date'] <= date("Y-m-d")) {
                    $expiry_date = "<span class='ExpiryDateExpired'><b>{$expiry_date}</b></span>";
                }
            }
            ...
            $NoStockText = "";
            if($stock <= 0) {
                $NoStockText = "<span class='blinking'><b> - Out Of Stock</b></span>";
            ...
            $product_name = "<a class='productNameInOrderItemWithLink' href='index.php?_topRm=pharmacy&module=tradingin_inventory&_action=edit&record_id={$row['record_id']}' target='_blank'>
                                <u>{$row['item_title']}</u>
                            </a>";

            if($row['po_product_id'] == "") {
                $product_name = $row['item_title'];
                $NoStockText = "<span class='blinking'><b> - No Stock Added</b></span>";
            ...
            $inputRow = "";
            if($row['not_add_in_stock'] == 1){
                $notStockCheckBox = "checked=checked";
                $inputRow = "<input class='orderItemIdNotAddInStock' {$notStockCheckBox} type='checkbox' name='orderItemId' value='{$row['order_item_id']}'>";
            }
            ...
            AND ms.site_id = {$rowsitedetail['site_id']}
            ";
            ...
            <td align='center'>{$inputRow}</td>
            <td class='txtCenter'>{$rowMS['rake']}</td>
            <td class='w25p' align='left'>{$product_name} {$NoStockText}</td>
            <td>{$row['batch_no']}</td>
            <!--<td>{$product_code}</td>-->
            <!--<td class='txtCenter'>{$row['unit']}</td>-->
            <td class='{$packSizeColumn}' align='center'>{$row['pack_size']}({$row['unit_price']}Rs)</td>
    ...
    ******************************* END PROCESS **********************************/
    function getGenerateBill() {
        ...
        $session_order_id   = isset($_SESSION['order_id']) ? $_SESSION['order_id']  : '';
        $orderRec = $fn->getRecordByCondition('order', "order_id = '{$session_order_id}'");
        $discountValueTotal = '';
        $invoice_amount     = '';
        $discount = 0;
        ...
        $fa['patient_visit_id'] = $orderRec['patient_visit_id'];
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if($row['not_add_in_stock'] == 0 || $row['not_add_in_stock'] == "") {
                $sqlAppendSt = '';
                $siteIdForField = "";
                if ($cpCfg['cp.hasMultiUniqueSites']) {
                    $site_id        = $fn->getSessionParam('cp_site_id');
                    $sqlAppendSt    = "AND site_id = {$site_id}";
                ...
                if($numRowsBatchWiseStock > 0) {
                    $faStock = array();
                    $faStock['sales_qty']     = $BatchWiseStock['sales_qty'] + $row['qty'];
                    $faStock['current_stock'] = $BatchWiseStock['current_stock'] - $row['qty'];
                    
                    $faStock        = $fn->addModificationDetailsToFieldsArray($faStock, 'inventory_batchwise_stock');
                    $whereCondition = "WHERE po_product_id = '{$row['po_product_id']}'
                                       AND product_id = '{$row['record_id']}'
                                       {$sqlAppendSt}";
                    ...
                    $SQLUpdateProduct = "
                    UPDATE product SET qty_in_stock{$siteIdForField} = IFNULL(qty_in_stock{$siteIdForField}, 0)-{$row['qty']}
                    WHERE product_id = '{$row['record_id']}'
                    ";
                    $resultUpdateProduct  = $db->sql_query($SQLUpdateProduct);
                    
                    $SQLUpdateInventory = "
                    UPDATE inventory SET actual_stock{$siteIdForField} = IFNULL(actual_stock{$siteIdForField}, 0)-{$row['qty']}
                    WHERE product_id = '{$row['record_id']}'
                    ";
   ...
   function getInsertOldOrder() {
    ...
    function getPrintInvoiceRecord() {
        ...
        SELECT oi.*
              ...
              ,o.visit_code
              ,o.in_patient_id
              ,DATE_FORMAT(i.creation_date, '%d-%m-%Y')AS invoice_creation_date
              ,DATE_FORMAT(i.creation_date, '%h:%i:%s %p')AS invoice_creation_time
              ,i.vat AS invoice_vat
              ,i.invoice_date
              ,i.invoice_code_vat
              ,i.invoice_code
        ...
        if ($cpCfg['cp.hasMultiUniqueSites']) {
            if($cpSiteIdSession == 1){
                $cpCfg['cp.phonePDF'] = 'KURUKKUSALAI, THOOTHUKUDI - 628 722';
            }
            else if($cpSiteIdSession == 2){
                $cpCfg['cp.phonePDF'] = 'THOOTHUKUDI - 628 002';
            }

        }

        $ref_by_dr = "";
        if($company['ref_by_dr'] == "Dr Meenakshi Sundaram" || 
           $company['ref_by_dr'] ==  "DR.SHEIK ABDUL KHADER" ||
           $company['ref_by_dr'] ==  "DR.KANNAN" ) {
            $ref_by_dr = $company['ref_by_dr'];
        }

        if($company['visit_code'] != ""){
            $orderIdHead = $orderIdHead.' / V.CODE: '.$company['visit_code'];
        }

        if($company['in_patient_id'] > 0){
            $ipRec = $fn->getRecordRowByID('in_patient', 'in_patient_id', $company['in_patient_id']);
            $orderIdHead = $orderIdHead.' / IP: '.$ipRec['code'];
        }
        ...
        <table border="0" width="100%" style="border-bottom:2px solid #000000;">
            <tr>
                <td width="100%" align="left"><br/>
                    <span style="font-size:18px;"><b>'.$cpCfg['cp.addressPharmacyPdf1'] .'</b></span><br/>
                    <span style="font-size:14px;">'.$cpCfg['cp.phonePDF'] .'<br/></span>
                    <span style="font-size:10px;">'.$cpCfg['cp.addressPharmacyPdf3'].'&nbsp;&nbsp;&nbsp;'.$cpCfg['cp.addressPharmacyPdf4'].'<br/></span>
                    <span style="font-size:14px;">'.$cpCfg['cp.doctorNamePDF'] .'<br/></span>
                </td>
            </tr>
        </table>';

        $header .= '
        <table border="0" width="100%" cellpadding="2"><br/>
            <tr style="line-height:25px;">
                <td style="font-size:18px;" align="center"><u>BILL</u></td>
            </tr>
            <tr>

                <td style="font-size:12px;" align="left" width="50%">NAME: '.$company['cust_first_name'].'</td>
                <td style="font-size:10px;" align="right" width="50%">DATE: '.$company['invoice_creation_date'].' '.$company['invoice_creation_time'].'</td>
            </tr>
            <tr>
                <td style="font-size:12px;" align="left" width="50%">Age: '.$company['age_year'].'</td>
            </tr>
            <tr>
                <td style="font-size:12px;" align="left" width="50%">Ref By: '.$ref_by_dr.'</td>
                <td style="font-size:10px;" align="right" width="50%">BILL NO: '.$orderIdHead.'</td>
            </tr>
        </table>
        ';
        ...
        $tbl2 ='<table border="0" width="100%" cellpadding="5"><br/><br/>
                    <tr>
                        <th width="8%"  align="center" style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">Qty</th>
                        <th width="35%" align="left"   style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">PARTICULARS</th>
                        <th width="16%" align="left"   style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">BATCH</th>
                        <th width="13%" align="left"   style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">EXP.DT</th>
                        <th width="12%" align="right"  style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">VALUE</th>
                        <th width="16%" align="right"  style="font-size:12px;border-top:1px dashed black;border-bottom:1px dashed black;">AMOUNT</th>
                    </tr>
        ';
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $tbl2 = $tbl2.'
                    <tr>
                        ...
                        <td width="35%" style="font-size:12px;" align="left">'.$row['product_title'].'</td>
                        <td width="16%" align="left">'.$row['batch_no'].'</td>
                        <td width="13%" align="left">'.$expiry_date.'</td>
                        <td width="12%" style="font-size:12px;" align="right">'.number_format($unit_price,2).'</td>
                        <td width="16%" style="font-size:12px;" align="right">'.number_format($total, 2).'</td>
                    </tr>
            ';
        ...
        $tbl3 = $tbl3.'
                <tr>
                    <td align="left" style="border-top:1px dashed black;line-height:10px;"><font style="font-size:12px;">SUB TOTAL:</font></td>
                    <td align="right" colspan="2" style="border-top:1px dashed black;line-height:10px;"><font style="font-size:12px;">'.number_format($totalNetAmount, 2).'</font></td>
                </tr>
                <tr>
                    <td align="left" style="border-bottom:1px dashed black;line-height:10px;"><font style="font-size:12px;">ROUND OFF:</font></td>
                    <td align="right" colspan="2" style="border-bottom:1px dashed black;line-height:10px;"><font style="font-size:12px;">'.number_format($totalAmount - $totalNetAmount, 2).'</font></td>
                </tr>
                <tr>
                    <td align="left" style="border-bottom:1px dashed black;line-height:20px;"><font style="font-size:15px;">TOTAL:</font></td>
                    <td align="right" colspan="2" style="border-bottom:1px dashed black;line-height:20px;"><font style="font-size:15px;">'.number_format($totalAmount, 2).'</font></td>
                </tr>
                <tr>
                    <td align="left"  width="30%"><font style="font-size:14px;">ITEM(S): '.$serialNoTotal.'</font></td>
                    <td align="center" width="20%"><font style="font-size:14px;">QTY: '.$total_qty.'</font></td>
                    <td align="right"  width="50%"><font style="font-size:15px;">RS. '.number_format($totalAmount, 2).'</font></td>
                </tr>
                <tr><br/><br/>
        ...
        $pdf = new MYPDF_Local('P', 'px', array('387.04', $heightPage), true, 'UTF-8', false);                
        //$pdf = new MYPDF_Local(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    function getBatchProductSelect() {
        ...
        while ($rowPo = $db->sql_fetchrow($resultPo)){
            ...
            $productNameRow = "<a class='batchProductAdd' batch_no='{$rowPo['batch_no']}' po_product_id='{$rowPo['po_product_id']}' product_id='{$rowPo['product_id']}'>
                                    {$rowPo['title']}
                                </a>";

            if($rowPo['expiry_date'] != "" && $rowPo['expiry_date'] != "0000-00-00") {
            ...
            LEFT JOIN (po_product pop) ON (pop.po_product_id = ibs2.po_product_id)
            LEFT JOIN (purchase_order pur) ON (pur.purchase_order_id = ibs2.purchase_order_id)
            WHERE ibs2.product_id = '{$rowPo['product_id']}'
              AND ibs2.inventory_batchwise_stock_id < '{$rowPo['inventory_batchwise_stock_id']}'
              AND ibs2.site_id = '{$rowPo['site_id']}'
              AND pur.status != 'Cancelled'
              AND ibs2.current_stock != 0
            ORDER BY ibs2.inventory_batchwise_stock_id ASC
            ";
            ...
            if($rowStockPrev['stockCheck'] < 0) {
                $rowPo['current_stock'] = $rowPo['current_stock'] + $rowStockPrev['stockCheck'];
            } else {
                if($numRows == 1) {
                    $SQLStockPrev2 ="
                    SELECT SUM(ibs2.current_stock) AS stockCheck
                    ...
                    $rowPo['current_stock'] = $rowPo['current_stock'] + $rowStockPrev2['stockCheck'];
                }
            }
            
            if($rowPo['current_stock'] > 0) {
                $rows .= "
                <tr {$bgColorChange}>
                    <td>{$productNameRow}</td>
                    <td>{$rowPo['batch_no']}</td>
                    <td class='txtCenter'><b>{$rowPo['current_stock']}</b></td>
                    <td class='txtRight'>{$selling_price}</td>
                    <td class='txtCenter'>{$rowPo['gst']}</td>
                    <td class='txtCenter'>{$rowPo['pack_size']}</td>
                    <td>{$expiry_date}</td>
                    {$poTd}
                </tr>
                ";
     ...
     *
     */
    function getBatchProductSelect1() {
        $db      = Zend_Registry::get('db');
        $fn      = Zend_Registry::get('fn');
        $tv      = Zend_Registry::get('tv');
        $cpCfg   = Zend_Registry::get('cpCfg');
        ...
        while ($rowPo = $db->sql_fetchrow($resultPo)){
            ...
            if($stock > 0 || $count == $numRows){

                $poTd = "";
                if($rowPo['stock_from'] == "PURCHASE ORDER") {
                ...
                <tr>
                    <td>{$productNameRow}</td>
                    <td>{$rowPo['batch_no']}</td>
                    <td class='txtCenter'><b>{$stock}</b></td>
                    <td class='txtRight'>{$selling_price}</td>
                    <td class='txtCenter'>{$rowPo['gst']}</td>
                    <td class='txtCenter'>{$rowPo['pack_size']}</td>
                    <td>{$expiry_date}</td>
