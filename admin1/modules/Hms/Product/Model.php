<?
class CPL_Admin_Modules_Hms_Product_Model extends CP_Admin_Modules_Hms_Product_Model
{
    ...
    function getSQL() {
        ...
        SELECT DISTINCT p.product_id
              ...
              ,p.title
              ,p.description
              ,p.qty_in_stock1 AS qty_in_stock
              ,p.price
              ,p.published
              ,p.creation_date
              ,p.modification_date
    ...
    function setSearchVar($linkRecType = '') {
        ...
        if ($product_id != '') {
            ...
            if ($special_search != '' ) {
                ...
                if ($special_search == 'View All') {
                    $searchVar->sqlSearchVar[] = "(p.published = 0 OR p.published IS NULL OR p.published = '' OR p.published = 1)";
                }*/
                ...
                if ($special_search == 'Not deduct in stock') {
            ...
            if($published != "") {
                $searchVar->sqlSearchVar[] = "p.published = {$published}";
            } else {
                $searchVar->sqlSearchVar[] = "p.published = 1";
            }
     ...
     *
     */
    function getUpdateProductCodeOld() {
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
    ...
    function getFields() {
        ...
        $fa = $fn->addToFieldsArray($fa, 'medicine_qty');
        $fa = $fn->addToFieldsArray($fa, 'days');
        $fa = $fn->addToFieldsArray($fa, 'instruction');
        $fa = $fn->addToFieldsArray($fa, 'not_add_in_stock');

        if(isset($_POST['published'])){
            $fa = $fn->addToFieldsArray($fa, 'published');
        }
        
        $fa = $fn->addToFieldsArray($fa, 'add_syringe_in_pos');
        $fa = $fn->addToFieldsArray($fa, 'modified_by');
        $fa = $fn->addToFieldsArray($fa, 'created_by');
        $fa = $fn->addToFieldsArray($fa, 'exclude_stock_difference');
    ...
    function getConsultantFormSubmit() {
        ...
        $insertAgeWiseSQL = $dbUtil->getInsertSQLStringFromArray($fa, 'consultant_doctor');
        $resultAgeWiseSQL = $db->sql_query($insertAgeWiseSQL);
    ...
    function getConsultantValidate() {
        ...
        $validate->resetErrorArray();
        $validate->validateData('employee_id', 'Please choose Consultant');
    ...
    function getDeleteConsultant(){
        ...
        $SQL ="
               DELETE FROM consultant_doctor
               WHERE consultant_doctor_id = {$consultant_doctor_id}
               ";
        $result = $db->sql_query($SQL);
    }
    ...
    function getDosageAgeWiseFormSubmit() {
        ...
        if (!$this->getDosageAgeWiseValidate()){
            return $validate->getErrorMessageXML();
        }
        ...
        $age_from        = $fn->getPostParam('age_from');
        $age_to          = $fn->getPostParam('age_to');
        $age_from_year   = $fn->getPostParam('age_from_year');
        $age_from_month  = $fn->getPostParam('age_from_month');
        $age_from_day    = $fn->getPostParam('age_from_day');
        $age_to_year     = $fn->getPostParam('age_to_year');
        $age_to_month    = $fn->getPostParam('age_to_month');
        $age_to_day      = $fn->getPostParam('age_to_day');


        $fa = array();

        if($age_to_year == "" && $age_from_year != ""){
            $age_to_year = $age_from_year;
        }

        $fa['dosage']          = $dosage;
        $fa['instruction']     = $instruction;
        $fa['age_from_year']   = $age_from_year;
        $fa['age_from_month']  = $age_from_month;
        $fa['age_from_day']    = $age_from_day;
        $fa['age_to_year']     = $age_to_year;
        $fa['age_to_month']    = $age_to_month;
        $fa['age_to_day']      = $age_to_day;
        $fa['product_id']      = $product_id;
        $fa['creation_date']   = date("Y-m-d H:i:s");
        $fa['created_by']      = $fn->getSessionParam('userName');

        $insertAgeWiseSQL = $dbUtil->getInsertSQLStringFromArray($fa, 'dosage_agewise');
        $resultAgeWiseSQL = $db->sql_query($insertAgeWiseSQL);
    ...
    function getEditDosageAgeWiseFormSubmit() {
        ...
        $age_from          = $fn->getPostParam('age_from');
        $age_to            = $fn->getPostParam('age_to');
        $dosage_agewise_id = $fn->getPostParam('dosage_agewise_id');
        $age_from_year     = $fn->getPostParam('age_from_year');
        $age_from_month    = $fn->getPostParam('age_from_month');
        $age_from_day      = $fn->getPostParam('age_from_day');
        $age_to_year       = $fn->getPostParam('age_to_year');
        ...
        $fa1['age_from_year']          = $age_from_year;
        $fa1['age_from_month']         = $age_from_month;
        $fa1['age_from_day']           = $age_from_day;
        $fa1['age_to_year']            = $age_to_year;
        $fa1['age_to_month']           = $age_to_month;
        $fa1['age_to_day']             = $age_to_day;
        $fa1['dosage_agewise_id']      = $dosage_agewise_id;
        $fa1['modification_date']      = date("Y-m-d H:i:s");
        $fa1['modified_by']            = $fn->getSessionParam('userName');

        $whereConditionAgeWise = "WHERE dosage_agewise_id = {$dosage_agewise_id}" ;
        $sqlUpdateAgeWise      = $dbUtil->getUpdateSQLStringFromArray($fa1, "dosage_agewise", $whereConditionAgeWise);
        $resultUpdateAgeWise   = $db->sql_query($sqlUpdateAgeWise);
    ...
    function getDosageAgeWiseValidate() {
        ...
        $validate->resetErrorArray();
        $validate->validateData('dosage', 'Please enter dosage');
    ...
    function getDosageWeightWiseFormSubmit() {
        ...
        if (!$this->getDosageWeightWiseValidate()){
            return $validate->getErrorMessageXML();
        }
        ...
        $dosage       = $fn->getPostParam('dosage');
        $instruction  = $fn->getPostParam('instruction');
        $wt_from      = $fn->getPostParam('wt_from');
        $wt_to        = $fn->getPostParam('wt_to');
        ...
        $insertWeightWiseSQL = $dbUtil->getInsertSQLStringFromArray($fa, 'dosage_wtwise');
    ...
    function getEditDosageWeightWiseFormSubmit() {
        ...
        $wt_from          = $fn->getPostParam('wt_from');
        $wt_to            = $fn->getPostParam('wt_to');
        $dosage_wtwise_id = $fn->getPostParam('dosage_wtwise_id');
        ...
        $fa1['wt_from']               = $wt_from;
        $fa1['wt_to']                 = $wt_to;
        $fa1['dosage_wtwise_id']      = $dosage_wtwise_id;
        $fa1['modification_date']     = date("Y-m-d H:i:s");
        $fa1['modified_by']           = $fn->getSessionParam('userName');

        $whereConditionWeightWise = "WHERE dosage_wtwise_id = {$dosage_wtwise_id}" ;
        $sqlUpdateWeightWise      = $dbUtil->getUpdateSQLStringFromArray($fa1, "dosage_wtwise", $whereConditionWeightWise);
    ...
    function getProductDetailsUpdateLink() {
        ...
        //http://habibiahms.localhost/admin/index.php?module=hms_product&_spAction=ProductDetailsUpdateLink&showHTML=0
        ...
        while($rowProduct = $db->sql_fetchrow($resultProduct)){
            $item_code = $this->getUpdateProductCode();
            ...
            $selling_price = $rowPoProduct['selling_price'];
            $gst           = $rowPoProduct['gst'];
            ...
            WHERE product_id = {$rowProduct['product_id']}
            ";
            $resultProductUpdate = $db->sql_query($SQLProductUpdate);
            $rowProductUpdate    = $db->sql_fetchrow($resultProductUpdate);
        }
    }
    ...
    function getUpdateProductCode() {
        ...
        $SQLProdCode = "
        SELECT MAX(CONVERT(REPLACE(item_code, 'PROD - ', ''), UNSIGNED INTEGER)) AS prod_code
        FROM `product`
        ";
        $resultProdCode = $db->sql_query($SQLProdCode);
        $rowProdCode    = $db->sql_fetchrow($resultProdCode);
        $prod_code      = $rowProdCode['prod_code'] + 1;

        if($rowProdCode['prod_code'] == "" || $rowProdCode['prod_code'] == 0) {
            $prod_code = "PROD - 1000";
        }
        else {
    ...
    function getAddBranchMedicineSubmit() {
        ...
        if (!$this->getBranchMedicineValidate()){
            return $validate->getErrorMessageXML();
        }

        $product_id  = $fn->getReqParam('product_id');
        $site_id     = $fn->getPostParam('site_id');
        $rake        = $fn->getPostParam('rake');
        $rack_qty        = $fn->getPostParam('rack_qty');

        $fa = array();
        $fa['product_id']    = $product_id;
        $fa['site_id']       = $site_id;
        $fa['rake']          = $rake;
        $fa['rack_qty']      = $rack_qty;
        $fa['creation_date'] = date("Y-m-d H:i:s");
        $fa['created_by']    = $fn->getSessionParam('userName');
    ...
    function getEditBranchMedicineSubmit() {
        ...
        $fa1['site_id']           = $site_id;
        $fa1['rake']              = $rake;
        $fa1['rack_qty']          = $rack_qty;
        $fa1['modification_date'] = date("Y-m-d H:i:s");
        $fa1['modified_by']       = $fn->getSessionParam('userName');

        $whereCondition = "WHERE medicine_site_id = {$medicine_site_id}";
        $SQL    = $dbUtil->getUpdateSQLStringFromArray($fa1, "medicine_site", $whereCondition);
        $result = $db->sql_query($SQL);
