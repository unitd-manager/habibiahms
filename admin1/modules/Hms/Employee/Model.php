<?
class CPL_Admin_Modules_Hms_Employee_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
    ...
    function setSearchVar($linkRecType = '') {
        ...
        if ($employee_id != "") {
            ...
            // else if($tv['keyword'] == "") {
            //     $searchVar->sqlSearchVar[] = "a.position = 'Nurse'";                
            // }
            ...
            // else{
            //     $searchVar->sqlSearchVar[] = "a.status = 'Active'";
            // }
    ...
    function getNewValidate() {
        ...
        if (count($validate->errorArray) == 0) {
            ...
            $validate->errorArray['error_box']['msg']  = "* Please fill mandatory fields";
            return false;
        }
    }
    ...
    function getEditValidate() {
        ...
        $validate->validateData('first_name', 'Please enter the first name');
        $validate->validateData('position', 'Please enter the position');
        //$validate->validateData('staff_id', 'Please select staff');
        $validate->validateData('category', 'Please select category');
        /*$validate->validateData('email', 'Please enter the email');
        $validate->validateData('pass_word', 'Please enter the pass word');*/
        //$validate->validateData('employee_work_type' , 'Please select the work type');

        $login_email = $fn->getPostParam('login_email');
        $staff_id = $fn->getPostParam('staff_id');
        $sqlCon = '';
        if($staff_id != ''){
            $sqlCon = "AND s.staff_id != {$staff_id}";
        }

        $SQLStaff ="
        SELECT s.staff_id
        FROM staff s
        WHERE s.email = '{$login_email}'
        {$sqlCon}  
        ";
        ...
        if ($numRowsStaff > 0){
            ...
            $validate->errorArray['login_email']['name'] = "login_email";
            $validate->errorArray['login_email']['msg']  = "Email id already exists";
        }
    ...
    function getSave(){
        ...
        if($fa['employee_type'] == 'Double Shift') {
            $time_in_night  = $fn->getPostParam('time_in_night1');
            $time_out_night = $fn->getPostParam('time_out_night1');

            $fa['time_in_night']  = $time_in_night;
            $fa['time_out_night'] = $time_out_night;
        }
        
        /*$SQLUG="
        SELECT ug.user_group_id
        FROM user_group ug
        WHERE ug.title = 'Doctor'
        ";
        $resultUG   = $db->sql_query($SQLUG);
        $rowUG = $db->sql_fetchrow($resultUG);

        $employee = $fn->getRecordRowByID('employee', 'employee_id', $employee_id);
        $staff_id = $employee['staff_id'];

        if($staff_id == ""){
            $fa1 = array();
            $fa1['email']         = $fa['email'];
            $fa1['pass_word']     = $fa['pass_word'];
            $fa1['user_group_id'] = $rowUG['user_group_id'];
            $fa1['creation_date'] = date("Y-m-d H:i:s");
            $fa1['created_by']    = $fn->getSessionParam('userName');

            $staff_id = $fn->addRecord($fa1, 'staff');

            $faemp = array();
            $faemp['staff_id']          = $staff_id;
            $faemp['modification_date'] = date("Y-m-d H:i:s");
            $faemp['modified_by']       = $fn->getSessionParam('userName');
            $whereCondition ="
            WHERE employee_id = {$employee['employee_id']}";
            $SQLEmp = $dbUtil->getUpdateSQLStringFromArray($faemp, 'employee', $whereCondition);
            $db->sql_query($SQLEmp);
        }
            ...
            $SQL = $dbUtil->getUpdateSQLStringFromArray($fa1, 'staff', $whereCondition);
            $db->sql_query($SQL);
        }*/

        $employeeRec = $fn->getRecordRowByID('employee', 'employee_id', $employee_id);
        $record_id = $fn->getIssetParam($employeeRec, 'employee_id');
        $current_date = date('Y-m-d');
        $cpSiteIdSession = $fn->getSessionParam('cp_site_id');

        if($fa['fees_commission'] != $employeeRec['fees_commission'] ){

        $fa2 = array();
        $fa2['comments']      = $employeeRec['fees_commission'];
        $fa2['room_name']     = 'hms_employee';
        $fa2['record_id']     = $record_id;
        $fa2['comment_date']  = $current_date;
        $fa2['creation_date'] = $current_date;
        $fa2['published']     = 1;
        $fa2['site_id']       = $cpSiteIdSession;

        $SQLInsert = $dbUtil->getInsertSQLStringFromArray($fa2, 'comment');
        ...
        $login_email = $fn->getPostParam('login_email');
        $login_pass_word     = $fn->getPostParam('login_pass_word');
        $staff_user_group_id = $fn->getPostParam('staff_user_group_id');
        $staff_published     = $fn->getPostParam('staff_published');

        if($login_email != "" && $login_pass_word != ''){
            if($staff_id != ''){
                $faStaff = array();
                $faStaff['first_name']  = $fa['first_name'];
                $faStaff['email']       = $login_email;
                $faStaff['pass_word']   = $login_pass_word;
                $faStaff['user_group_id']   = $staff_user_group_id;
                $faStaff['published']   = $staff_published;
                $whereCondition = "WHERE staff_id = {$staff_id}";
                $sqlUpdate = $dbUtil->getUpdateSQLStringFromArray($faStaff, "staff", $whereCondition);
                ...
                $faStaff['user_group_id']   = $staff_user_group_id;
                $faStaff['published']   = $staff_published;
                $faStaff['status'] = 'Current';
                $faStaff['site_id']       = $cpSiteIdSession;
                $faStaff['creation_date'] = date('Y-m-d H:i:s');
                $SQLInsert = $dbUtil->getInsertSQLStringFromArray($faStaff, 'staff');
                $resultInsert = $db->sql_query($SQLInsert);
                $staff_id = $db->sql_nextid();                

                $SQLUpdate ="
                UPDATE employee set staff_id = {$staff_id} WHERE employee_id = {$id}
                ";
                $resultUpdate = $db->sql_query($SQLUpdate);
            }
        }
    ...
    function getFields(){
        ...
        $fa = $fn->addToFieldsArray($fa, 'fees_commission_type');
        $fa = $fn->addToFieldsArray($fa, 'fees_commission');
        $fa = $fn->addToFieldsArray($fa, 'lab_commission_type');
        $fa = $fn->addToFieldsArray($fa, 'lab_commission');
        $fa = $fn->addToFieldsArray($fa, 'time_in_night');
        $fa = $fn->addToFieldsArray($fa, 'time_out_night');
        $fa = $fn->addToFieldsArray($fa, 'joining_date');
        $fa = $fn->addToFieldsArray($fa, 'employee_type');
        $fa = $fn->addToFieldsArray($fa, 'time_in_morning');
        $fa = $fn->addToFieldsArray($fa, 'time_out_morning');
        $fa = $fn->addToFieldsArray($fa, 'time_in_evening');
        $fa = $fn->addToFieldsArray($fa, 'time_out_evening');
     ...
     *
     */
    function getAddEmployeePerformanceValidate() {
        $validate = Zend_Registry::get('validate');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        ...
        $validate->validateData('type', 'Please select type');
        $validate->validateData('date', 'Please select date');
        $validate->validateData('rating', 'Please select rating');
    ...
    function getAddEmployeePerformanceSubmit() {
        ...
        if (!$this->getAddEmployeePerformanceValidate()){
            return $validate->getErrorMessageXML();
        }

        $employee_id = $fn->getReqParam('employee_id');
        $type        = $fn->getPostParam('type');
        $date        = $fn->getPostParam('date');
        $rating      = $fn->getPostParam('rating');
        ...
        $insert = $dbUtil->getInsertSQLStringFromArray($fa, 'emp_performance');
        $result = $db->sql_query($insert);
    ...
    function getEditEmployeePerformanceSubmit() {
        ...
        $whereCondition = "WHERE emp_performance_id = {$emp_performance_id}";
        $SQL    = $dbUtil->getUpdateSQLStringFromArray($fa, "emp_performance", $whereCondition);
        $result = $db->sql_query($SQL);
    ...
    function getDeleteEmployeeRecord() {
        ...
        $SQL1 = "DELETE FROM `staff` WHERE staff_id = {$employee['staff_id']}";
