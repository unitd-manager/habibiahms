<?
class CPL_Admin_Modules_Hms_Invoice_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
     ...
     */
     function getGenerateInvoiceForm() {
            ...
            while($rowOrderItem  = $db->sql_fetchrow($resultOrderItem)){
                ...
                if($numRowsList > 0){
                    ...
                    while($rowList    = $db->sql_fetchrow($resultList)){
                        ...
                        if($rowOrderItem['record_type'] == 'Vaccination'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Room Rent'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Nursing Fees'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Other Fees'){
                            $disabledInput = "disabled = ''";
                        }


                        if($rowOrderItem['record_type'] == 'Consultant Fees'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Oxygen Fees'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Medicine Fees'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Advance Payment'){
                            $disabledInput = "disabled = ''";
                        }

                        if($rowOrderItem['record_type'] == 'Doctor/Nurse'){
                           if($rowList['item_title'] == ""){
                               $rowList['item_title'] = "Nurse";
                           }
                           $disabledInput = "disabled = ''";
                        }

                        $List_fees_input            = "<input name='list_fees_{$count}' class='order_item_type_value list_fees_{$count}'  value='{$List_Amount}' disabled/>";
                    ...
                    if($row['order_type'] == 'IP' && $rowOrderItem['record_type'] == 'Lab Test') {
                        $labFeesAmount  = number_format($labFeesAmount, 2);
                        $Lab_fees_input = "<input name='lab_fees' class='order_item_type_value lab_fees' value='{$labFeesAmount}' disabled/>";
                        $order_items_Details .="<table class='thinlist InvoiceFormTable'>
                                                    <tr>
                                                        <th width='80%'>{$rowOrderItem['record_type']}</th>
                                                        <td width='20%' class='txtRight'>{$Lab_fees_input}</td>
                                                    <tr>
                                                </table>
                                                ";
                    } else {
        ...
        $order_items_Details .="
        <div class='order_item_type_title'></div>
        <table class='thinlist order_item_type_table'>
             <tr>
                <th width='75%'>Total Amount</th>
