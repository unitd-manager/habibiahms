<?
class CPL_Admin_Modules_Hms_Invoice_Model extends CP_Common_Lib_ModuleModelAbstract
{
    /**
     *
     */
        ...
        if($numRowsInvoiceCheck == 0) {
            for ($i= 1; $i <= $total_count; $i++) {
                ...
                if($list_fees != '' && $list_orderItemId != ''){
                    ...
                    $fa1['not_add_in_stock']      = 0;

                    if($orderRec['order_type'] == 'IP'){
                        $SQLEmpVisit = "
                        SELECT consultation_fees
                               ,employee_in_patient_id
                               ,in_patient_id
                        FROM employee_in_patient
                        WHERE in_patient_id = {$orderRec['in_patient_id']}
                    ...
                    $faEV = array();
                    $faEV['consultation_fees'] = $list_fees;

                    if($orderRec['order_type'] == 'IP'){
                        $whereCondition = "
                        WHERE in_patient_id = {$rowEmpVisit['in_patient_id']}
                        AND employee_in_patient_id  = {$rowEmpVisit['employee_in_patient_id']}
                        ";
                        $SQLEV = $dbUtil->getUpdateSQLStringFromArray($faEV, 'employee_in_patient', $whereCondition);
                        $db->sql_query($SQLEV);
                    } else {
                        $whereCondition = "
                        WHERE patient_visit_id = {$rowEmpVisit['patient_visit_id']}
                        AND employee_visit_id  = {$rowEmpVisit['employee_visit_id']}
                        ";
            ...
            if($Total_Amount >= 0){
                ...
                $invoiceRec1       = $fn->getRecordRowByID('invoice', 'invoice_id', $rowInvoiceCode['invoice_id']);
                $invoice_code      = (int)(str_replace('INV - ', '', $invoiceRec1['invoice_code'])) + 1;
                ...
                $fa['invoice_type'] = $orderRec['order_type'];
                ...
                if($orderRec['order_type'] == 'OP'){
                    ...
                    $SQLUpdate = "
                    UPDATE in_patient SET status = 'Bill Due'
                    , order_id = {$order_id}
                    WHERE in_patient_id = {$orderRec['in_patient_id']}
                    ";
                    $resultUpdate = $db->sql_query($SQLUpdate);
                    ...
                    $SQLUpdate = "
                    UPDATE lab_test SET status = 'Bill Due'
                    , order_id = '{$order_id}'
                    WHERE lab_test_id = '{$orderRec['lab_test_id']}'
                    ";
                    $resultUpdate = $db->sql_query($SQLUpdate);                
                }
            }
            ...
            if($receipt == 1 && $Total_Amount >= 0){            
                ...
                $rowReceiptCode    = $db->sql_fetchrow($resultReceiptCode);
                $receiptRec1       = $fn->getRecordRowByID('receipt', 'receipt_id', $rowReceiptCode['receipt_id']);
                $receipt_code      = (int)(str_replace('RCPT - ', '', $receiptRec1['receipt_code'])) + 1;
                ...
                $count = 0;
                    ...
                    if($invoice_id != ''){
                        ...
                        $appendSqlPaid2 = '';
                        if ($cpCfg['cp.hasMultiUniqueSites']) {
                            $appendSqlPaid1 = " AND i.site_id = {$cpSiteIdSession}";
                ...
                if($order_amount == $total_invoice_amount){
                    ...
                    }elseif ($orderRec['order_type'] == 'IP') {
                        $SQLPVUpdate = "UPDATE in_patient SET status = 'Closed' WHERE in_patient_id = {$orderRec['in_patient_id']}";
                    } else {
                        $SQLPVUpdate = "UPDATE lab_test SET status = 'Closed' WHERE lab_test_id = '{$orderRec['lab_test_id']}'";                    
                    }
                    ...
                    if($orderRec['order_type'] == 'OP'){
                        ...
                        $SQLPVUpdate = "UPDATE in_patient SET status = 'Partial Receipt' WHERE in_patient_id = {$orderRec['in_patient_id']}";
                    } else {
                        $SQLPVUpdate = "UPDATE lab_test SET status = 'Partial Receipt' WHERE lab_test_id = '{$orderRec['lab_test_id']}'";                    
                    }
                    $resultPVUpdate = $db->sql_query($SQLPVUpdate);
                }
                ...
                if($orderRec['order_type'] == 'OP'){
                    ...
                    $SQLUpdate = "
                    UPDATE in_patient SET status = 'Closed'
                    , order_id = '{$order_id}'
                    WHERE in_patient_id = '{$orderRec['in_patient_id']}'
                    ";
                } else {
                    $SQLUpdate = "
                    UPDATE lab_test SET status = 'Closed'
                    , order_id = '{$order_id}'
                    WHERE lab_test_id = '{$orderRec['lab_test_id']}'
                    ";
                }
     ...
     *
     */
    function getGenerateInvoiceFormSubmitavv() {
        $fn = Zend_Registry::get('fn');
        $validate = Zend_Registry::get('validate');
        $db = Zend_Registry::get('db');
        $dbUtil = Zend_Registry::get('dbUtil');
        ...
        if($numRowsInvoiceCheck == 0) {
            ...
            FROM order_item
            WHERE order_id = '{$order_id}'
            AND record_type = 'Admission Details'
            GROUP BY record_type
            ORDER BY record_type ASC
            ";
            $resultOrderItem = $db->sql_query($SQLOrderItem);
