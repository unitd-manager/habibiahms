<?
class CPL_Admin_Modules_Hms_MedicalTest_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        {$listObj->getListHeaderCell('Category', 'mt.category')}
        {$listObj->getListHeaderCell('Fees', 'mt.fees' )}
        {$listObj->getListHeaderCell('Reference Value', 'mt.normal_value' )}
    ...
    function getEdit($row){
        ...
        $SQLInvestigationContainer   = $fn->getValueListSQL('investigationsContainer');        

        //$sqlCountry = getCPModelObj('common_geoCountry')->getCountryDDSQL();
        //$expCountry = array('detailValue' => $row['country_name']);

        $expVl = array('sqlType' => 'OneField');
        $formAddPosition = "index.php?_topRm={$tv['topRm']}&module=hms_medicalTest&_spAction=addNewValuelistForm&valuelist_name=investigationGroup&medical_test_id={$row['medical_test_id']}&showHTML=0";
                    ...
                    <table class='thinlist'>
                        <tbody>
                            <tr>
                                ...
                                <td>{$formObj->getTBRow('Reference Value', 'normal_value', $row['normal_value'])}</td>
                                <td>{$formObj->getTBRow('Units', 'units', $row['units'])}</td>
                            </tr>

                            <tr>
                                <td>{$formObj->getYesNoRRow('Blood- Urine Related Test', 'blood_related', $row['blood_related'])}</td>
                                <td>{$formObj->getDDRowBySQL('Category', 'category', $SQLInvestigationCategory, $row['category'], $expVl)}</td>
                                <td>{$formObj->getDDRowBySQL('Group Name', 'group_name', $SQLInvestigationGroupName, $row['group_name'], $expGroup)}</td>
                                <td>{$formObj->getTBRow('Lab Supplier Fees', 'lab_supplier_fees', $row['lab_supplier_fees'])}</td>
                                <td>{$formObj->getDDRowBySQL('Container', 'container', $SQLInvestigationContainer, $row['container'], $expVl)}</td>
                            </tr>
    ...
    function getRightPanel($row){
        ...
        $text = "
        <div id='medicalTestLinkPortal'>{$this->getMedicalTestParameter($record_id)}</div>
        {$media->getRightPanelMediaDisplay('Attachments', 'hms_medicalTest', 'attachment', $row)}
        ";
    ...
    function getMedicalTestParameter($medical_test_id=''){
        ...
        $formActionMedicalParameters = "index.php?module=hms_medicalTest&_spAction=medicalParameters&medical_test_id={$medical_test_id}&showHTML=0";
    ...
    function getMedicalTestParameterDetail($medical_test_id = ''){
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $formActionEditMedicalParameters   = "index.php?module=hms_medicalTest&_spAction=editMedicalParameters&medical_test_parameter_id={$row['medical_test_parameter_id']}&medical_test_id={$medical_test_id}&showHTML=0";
    ...
    function getMedicalParameters() {
        ...
        $formAction = "index.php?_topRm=main&module=hms_medicalTest&_spAction=medicalParametersFormSubmit&showHTML=0";
    ...
    function getEditMedicalParameters() {
        ...
        $formAction = "index.php?module=hms_medicalTest&_spAction=editMedicalParametersFormSubmit&showHTML=0&medical_test_parameter_id={$medical_test_parameter_id}";
    ...
    function getAddNewValuelistForm() {
        ...
        $formAction = "index.php?_topRm={$tv['topRm']}&module=hms_medicalTest&_spAction=addNewValuelistFormSubmit&showHTML=0";
