...
cpm.tradingsg.purchaseOrder = {
    init: function(){
        ...
        .livequery(cpm.tradingsg.purchaseOrder.poProductTitleEdit);
        ...
        $('#AddNewProduct').live('click', function (e){
            ...
            Util.openFormInDialog.call(this, 'addNewproductForm', title, 1000, 'auto', expObj);
        });
        ...
        $('.viewMedicines').live('click', function (e){
            e.preventDefault();
            var expObj = {
            ...
            Util.openDialogForLink.call(this, 'List of Medicines', 660, 450, expObj);
        });

        $('.allQtyDetails').live('click', function (e){
            e.preventDefault();
            ...
            Util.openDialogForLink.call(this, title, 580, 200, expObj);
        });
        ...
        $(".creationModificationPo").livequery('click', function (e){
            ...
            var url = "index.php?module=tradingsg_purchaseOrder&_spAction=creationModificationPO&po_product_id="+po_product_id+"&showHTML=0";
            var exp = {
                url: url
            };

            Util.openDialogForLink('Creation / Modification Details',  500, 150, 0, exp);
        });
        ...
        $(".applyCustomerDiscount").livequery('click', function (e){
            var overall_discount  = $('#fld_overall_discount').val();
            var purchase_order_id = $('#record_id').val();
            var url = "index.php?module=tradingsg_purchaseOrder&_spAction=updateOverallDiscount&showHTML=0";
            
            Util.showProgressInd();
            $.get(url, {purchase_order_id: purchase_order_id, overall_discount:overall_discount}, function(data){
                $('.grandTotalPurchasePo').html(data.grand_total);
                $('.disountTotalValuePurchaseOrderRightPanel').html(data.total_discount);
                Util.hideProgressInd();
            }, 'json');
        });

        $(".purchaseReturn").livequery('click', function (e){
            var purchase_return  = $('#fld_purchase_return').val();
            var purchase_order_id = $('#record_id').val();
            var url = "index.php?module=tradingsg_purchaseOrder&_spAction=updatePurchaseReturn&showHTML=0";
            Util.showProgressInd();
            $.get(url, {purchase_order_id: purchase_order_id, purchase_return:purchase_return}, function(data){
                $('.grandTotalPurchasePo').html(data.grand_total);
                $('.disountTotalValuePurchaseOrderRightPanel').html(data.total_discount);
                Util.hideProgressInd();
            }, 'json');
        ...
        $(".m-tradingsg_purchaseOrder #frmEdit select[name=status]").livequery('change', function (e){
            var status = $(this).val();
            var purchase_order_id   = $('#record_id').val();
            var purchaseOrderStatus = $("input[name='purchase_order_status']").val();

            if(status == "Cancelled") {
                var msg = "Do you want to cancel the purchase order this will reduce the stock? Also please delete the product added to stock";
                if (confirm(msg)){
                    Util.showProgressInd();
                    var url = "index.php?module=tradingsg_purchaseOrder&_spAction=checkSoldQtyInBatchWiseStock&showHTML=0";
                    $.get(url, {purchase_order_id: purchase_order_id}, function(data){
                        if(data > 0) {
                            alert("If you want to cancel please delete the products which has status closed!");
                            $(".m-tradingsg_purchaseOrder #frmEdit select[name=status]").val(purchaseOrderStatus);        
                        }
                        Util.hideProgressInd();
                    });
                } else {
        ...
        /* Return Qty PO Product */
        $('#productLinkPortal a.returnQtyPoProductNoStock').live('click', function (e){
            var title             = "Return No Stock  Qty";
            var purchase_order_id = $(this).attr('purchase_order_id');
            ...
            var expObj = {
               ...
               ,callbackOnSuccess: function(){
                    Util.closeAllDialogs();
                    var msg = 'Return Qty Updated Successfully';
                    Util.alert(msg, function(){
                        cpm.tradingsg.purchaseOrder.reloadPoProductLink(purchase_order_id);
                    });
                }
            }
            Util.openFormInDialog.call(this, 'returnQtyPoProductNS', title, 'auto', 'auto', expObj);
        });

        /* Return Qty PO Product */
        $('#productLinkPortal a.returnQtyPoProduct').live('click', function (e){
            var title             = "Return Qty";
            var purchase_order_id = $(this).attr('purchase_order_id');
            ...
            Util.openFormInDialog.call(this, 'returnQtyPoProduct', title, 'auto', 'auto', expObj);
        });

        $(".returnQtyPoProduct input[name=return_qty]").live('keyup', function (e){
            var return_qty = $(this).val();
            var stock      = $('#returnQtyPoProduct input[name=stock]').val();
            
            /*if(parseInt(stock) > 10) {
                if(parseInt(return_qty) > 10) {
                    Util.alert("Please Enter Return Qty Less Than Or Equal To 10");
                    $(".returnQtyPoProduct input[name=return_qty]").val(1);
                    $(".returnQtyPoProduct input[name=return_qty]").focus();
                }
            } else {*/
                if(parseInt(return_qty) > parseInt(stock)) {
                    Util.alert("Please Enter Return Qty Less Than Or Equal To "+stock);
                    $(".returnQtyPoProduct input[name=return_qty]").val(stock);
                    $(".returnQtyPoProduct input[name=return_qty]").focus();
                }
            //}
        });

        $("form#addNewproductForm input.itemPrice").livequery('change', function (e){
            var parent     = $(this).closest('tr');
            var product_id = $('input.product_id_hidden', parent).val();
            var pack_size  = $('input.packSize', parent).val();
            var price      = $('input#price', parent).val();
            var qty        = $('input.poQuantity', parent).val();
            
            if(pack_size == "") {
                var mgsalert = "Please Enter Pack Size";
                var n = noty({
                    text: mgsalert,
                    type: 'information',
                    dismissQueue: true,
                ...
                $('input.packSize', parent).focus();
            } else if(qty == "" || qty == 0) {
                var mgsalert = "Please Enter Qty";
                var n = noty({
                    text: mgsalert,
                    type: 'information',
                    dismissQueue: true,
                ...
                $('input.poQuantity', parent).focus();
            } else {
                Util.showProgressInd();
                var url = "index.php?module=tradingsg_purchaseOrder&_spAction=checkPricePercentage&showHTML=0";
                $.get(url, {product_id: product_id, pack_size: pack_size, price: price}, function(html){
                    if(html != "") {
                        var mgsalert = html;
                        var n = noty({
                            text: mgsalert,
    ...
    poProductTitleEdit: function() {
        ...
        $(titleObj).autocomplete({
            ...
            ,select: function(event, ui) {
                var selectedObj    = ui.item;
                var product_id     = selectedObj.id;
                $("input[name='product_id_edit_hidden']").val(product_id);
            }
        });
    },
    ...
    poProductTitleList: function() {
        ...
        $(titleObj).autocomplete({
            ...
            ,select: function(event, ui) {
                ...
                $("input[name='price[]']", parent).val(price);
                $("input[name='gst[]']", parent).val(gst);
                $("input[name='cost_price[]']", parent).val(cost_price);
                //$(this).after("<input type='hidden' name='product_id[]' value=" + product_id + ">");
            }
